...
// digital_ucie
// -----------------------------------------------------------------------------
// The fully-DIGITAL UCIe PHY: identical in composition to UCIe_PHY, but with the
// analog parts removed. The two leaf blocks that bundled analog SerDes/clocking
// inside UCIe_PHY are replaced by their digital-only counterparts:
//
//   UCIe_PHY                         digital_ucie
//   --------                         ------------
//   unit_mb_die  (digital + analog)  ->  digital_mb        (digital only)
//   SideBand_Top (digital + analog)  ->  digital_sideband  (digital only)
//   MainSM       (digital)           ->  MainSM            (unchanged)
//   Reg_File     (digital)           ->  Reg_File          (unchanged)
//
// Everything analog (MB PLL + clkdiv + clock-gates + SerDes + tri-state, and the
// SB PLL + ClkDiv + ser/des) now lives OUTSIDE this module in the analog hard
// macro (analog_hard_macro = mainband_analog_hard_macro + sideband_analog_hard_macro).
// A higher-level top instantiates digital_ucie alongside analog_hard_macro and
// wires the two together through the boundary ports below.
//
//  Boundary with the analog hard macro
//  -----------------------------------
//   * Clocks are now INPUTS sourced by the hard macro: lclk, gated_lclk, pll_clk,
//     pll_period (MainBand) and clk_sb (Sideband).
//   * MainBand TX parallel datapath leaves on the o_mb_* ports (serializer enables
//     + reversed lane words + valid word + pre-tri-state clk/track) and the MB RX
//     recovered parallel words arrive on the i_mb_* ports. The analog-only MB
//     controls generated by MainSM (PLL speed, clock-gate enable, the four
//     tri-state lane selects, the RX deserializer enables, vcmp enable/done) are
//     exposed on the o_mb_* ports too.
//   * Sideband SerDes parallel bus leaves on o_sb_ser_* and arrives on i_sb_des_*;
//     the serializer pattern_mode / pmo_en controls are exposed on o_sb_*.
//   * The raw forwarded MB RX clock/track pins (i_RCKP_P / i_RCKN_P / i_RTRK_P) are
//     still system inputs here because the digital clock-pattern detector samples
//     them directly (the hard macro also taps i_RCKP_P for RX sampling).
//   * The serialized MB/SB physical pins (i_RD_P, o_TD_P, RXCKSB, ...) are NOT here
//     - they belong to the analog hard macro.
//
//  This is a pure structural split: every digital net is wired identically to
//  UCIe_PHY. No functional behaviour is changed.
//  Simulation only.
// =============================================================================
...
module digital_ucie #(
    ...
    // =========================================================================
    // Clocks from the analog hard macro
    // =========================================================================
    input  logic                             lclk,           // MB RDI lclk (pll/16)
    input  logic                             gated_lclk,      // MB gated lclk
    input  logic                             pll_clk,         // MB gated PLL clock
    input  logic                             clk_sb,          // SB /8 parallel clock
    ...
    // =========================================================================
    // MainBand <-> analog hard macro (parallel datapath + analog controls)
    // =========================================================================
    // -- TX parallel datapath (digital -> hard-macro serializers / tri-state) --
    output logic                             o_mb_lfsr_ser_en,
    output logic [DATA_WIDTH_MB-1:0]         o_mb_lfsr_lane [0:NUM_LANES-1],
    output logic                             o_mb_valid_ser_en,
    output logic [DATA_WIDTH_MB-1:0]         o_mb_valid_word,
    output logic                             o_mb_tckp_p_pre,
    output logic                             o_mb_tckn_p_pre,
    output logic                             o_mb_ttrk_p_pre,
    // -- analog-only TX controls (MainSM -> hard macro) --
    output logic [2:0]                       o_mb_pll_speed_sel,
    output logic                             o_mb_lclk_g,
    output logic [1:0]                       o_mb_tx_data_lane_sel,
    output logic [1:0]                       o_mb_tx_val_lane_sel,
    output logic [1:0]                       o_mb_tx_clk_lane_sel,
    output logic [1:0]                       o_mb_tx_trk_lane_sel,
    // -- analog-only RX controls (MainSM/digital MB -> hard macro) --
    output logic [NUM_LANES-1:0]             o_mb_rx_data_deser_en,
    output logic                             o_mb_rx_valid_deser_en,
    output logic                             o_mb_vcmp_enable,
    output logic                             o_mb_vcmp_done,   // valid-cmp feedback -> deser
    // -- RX recovered parallel words (hard macro -> digital MB) --
    input  logic [DATA_WIDTH_MB-1:0]         i_mb_par_data [0:NUM_LANES-1],
    input  logic                             i_mb_data_valid,
    input  logic [DATA_WIDTH_MB-1:0]         i_mb_valid_frame_data,
    input  logic                             i_mb_valid_frame_vld,

    // =========================================================================
    // MainBand raw forwarded RX clock/track (digital clk detector samples these)
    // =========================================================================
    ...
    // =========================================================================
    // Sideband <-> analog hard macro (parallel SerDes bus + serializer controls)
    // =========================================================================
    output logic                             o_sb_pattern_mode,
    output logic                             o_sb_pmo_en,
    output logic [63:0]                       o_sb_ser_data_send,
    output logic                             o_sb_ser_vld_send,
    input  logic                             i_sb_ser_rdy,
    input  logic [63:0]                       i_sb_des_data_rcvd,
    input  logic                             i_sb_des_vld_rcvd,
    ...
    // digital_sideband's Reg_Access and the Reg_File.
    // =========================================================================
    logic [24:0] rf_addr;
    logic [7:0]  rf_be;
    ...
    // =========================================================================
    // MainBand die <-> MainSM / SideBand glue nets
    // =========================================================================
    ...
    logic        traffic_req_w;       // SideBand.traffic_req -> MainSM
    logic        clk_handshake_done;  // MainSM -> SideBand.traffic_rdy
    logic        rdi_lclk_g;          // MainSM -> hard-macro lclk_g

    // ---- LTSM macro-state ----
    LTSM_state_e          current_ltsm_state;  // MainSM macro state

    // ---- LTSM/RDI <-> MainBand die control/result wires ----
    logic                 mb_mapper_en;        // gated mapper enable -> digital_mb.i_mapper_en
    ...
    // =========================================================================
    // Drive the analog-bound MainBand controls out to the hard macro.
    // (these are generated by MainSM and consumed only by the hard macro,
    //  except mb_vcmp_enable which the digital valid comparator also uses)
    // =========================================================================
    assign o_mb_pll_speed_sel     = mb_pll_speed_sel;
    assign o_mb_lclk_g            = rdi_lclk_g;
    assign o_mb_tx_data_lane_sel  = mb_tx_data_lane_sel;
    assign o_mb_tx_val_lane_sel   = mb_tx_val_lane_sel;
    assign o_mb_tx_clk_lane_sel   = mb_tx_clk_lane_sel;
    assign o_mb_tx_trk_lane_sel   = mb_tx_trk_lane_sel;
    assign o_mb_rx_data_deser_en  = mb_rx_data_deser_en;
    assign o_mb_rx_valid_deser_en = mb_rx_valid_deser_en;
    assign o_mb_vcmp_enable       = mb_vcmp_enable;
    assign o_mb_vcmp_done         = mb_vcmp_done;

    // Sideband serializer controls out to the hard macro.
    assign o_sb_pattern_mode      = sbinit_pattern_mode;
    assign o_sb_pmo_en            = reg_PMO_enable_status;

    // =========================================================================
    // 1. Digital MainBand (TX/RX, no analog SerDes/clocking)
    // =========================================================================
    digital_mb #(
        ...
        // clocks from hard macro
        .i_gated_lclk         (gated_lclk),
        .i_pll_clk            (pll_clk),
        ...
        // TX -> hard macro (parallel datapath)
        .o_lfsr_ser_en        (o_mb_lfsr_ser_en),
        .o_lfsr_lane          (o_mb_lfsr_lane),
        .o_valid_ser_en       (o_mb_valid_ser_en),
        .o_valid_word         (o_mb_valid_word),
        .o_tckp_p_pre         (o_mb_tckp_p_pre),
        .o_tckn_p_pre         (o_mb_tckn_p_pre),
        .o_ttrk_p_pre         (o_mb_ttrk_p_pre),
        ...
        // RX <- hard macro (recovered parallel words)
        .i_par_data           (i_mb_par_data),
        .i_data_valid         (i_mb_data_valid),
        .i_valid_frame_data   (i_mb_valid_frame_data),
        .i_valid_frame_vld    (i_mb_valid_frame_vld),

        // RX raw forwarded clock/track (clk detector)
        .i_RCKP_P             (i_RCKP_P),
        .i_RCKN_P             (i_RCKN_P),
        .i_RTRK_P             (i_RTRK_P),
    ...
    // =========================================================================
    // 2. Digital SideBand (messaging + reg access + RDI, no analog ser/des)
    // =========================================================================
    digital_sideband #(
        ...
        .ser_data_send    (o_sb_ser_data_send),
        .ser_vld_send     (o_sb_ser_vld_send),
        .ser_rdy          (i_sb_ser_rdy),
        .des_data_rcvd    (i_sb_des_data_rcvd),
        .des_vld_rcvd     (i_sb_des_vld_rcvd),
    ...
    // -------------------------------------------------------------------------
    // Clocked in the clk_sb domain (same as Reg_Access inside digital_sideband).
    // =========================================================================
    Reg_File u_reg_file (
        .clk                  (clk_sb),
        .rst_n                (rst_n),
