...
//  2. Data Parity (ep) and Control Parity (cp) use Even Parity (XOR).
...
always_comb begin
    ...
    is_32bit_read = (req_hdr.req.opcode == SB_32_MEM_READ) || (req_hdr.req.opcode == SB_32_CFG_READ);
    is_64bit_read = (req_hdr.req.opcode == SB_64_MEM_READ) || (req_hdr.req.opcode == SB_64_CFG_READ);
    ...
    // [FIX]: Data Parity (even Parity -> XOR)
    // dp = XOR of payload bits for data-carrying completions; 0 otherwise
...
always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        // [FIX]: Control Parity (even Parity -> XOR)
        // Compute even parity over final header[62:0]
