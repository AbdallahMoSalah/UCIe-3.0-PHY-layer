    ...
    always @(posedge i_clk or negedge i_rst_n) begin
        if (!i_rst_n) begin
            ...
            case (mode_select)
                // ─────────────────────────────────────────────────────────────
                // ITER_128 mode:
                //   Accumulate bit-mismatches over 128 frames.
                //   PASS (detection_result=1) when total errors <= threshold.
                //   FAIL (detection_result=1) would be wrong; <= means "within budget".
                // ─────────────────────────────────────────────────────────────
                ITER_128: begin
                    ...
                    if (iteration_counter == MAX_ITERATIONS - 1) begin
                        // PASS when errors are within the allowed threshold
                ...
                // ─────────────────────────────────────────────────────────────
                // CONSEC_16 mode:
                //   Count consecutive complete valid frames
                //   (RVLD_L == VALID_PATTERN for the whole 32-bit word).
                //   PASS when MIN_CONSECUTIVE (16) frames seen back-to-back.
                // ─────────────────────────────────────────────────────────────
                CONSEC_16: begin
                    ...
                    if (consec_count >= MIN_CONSECUTIVE - 1) begin
                        detection_result <= 1'b1;
                    end else begin
                        detection_result <= 1'b0;
                    end
