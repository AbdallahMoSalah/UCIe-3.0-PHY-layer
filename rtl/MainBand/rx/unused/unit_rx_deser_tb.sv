...
//                                             → Data_Deserializer
//
//  Verifies that the data captured by the unit_data_deserializer exactly matches
...
module unit_rx_deser_tb;
    ...
    // RX-side signals
    // =====================================================================
    wire [DATA_WIDTH-1:0] valid_shift_reg;   // from unit_valid_deserializer
    wire                  valid_frame_pulse; // from unit_valid_frame_detector
    wire [DATA_WIDTH-1:0] rx_par_data;       // captured parallel data
    wire                  rx_data_valid;     // one-cycle valid pulse (mb_clk domain)

    // =====================================================================
    // TX: Valid-lane serializer  (uses existing unit_mb_serializer from unsued/)
    // =====================================================================
    ...
    // =====================================================================
    unit_data_deserializer #(.DATA_WIDTH(DATA_WIDTH)) u_data_des (
        .mb_clk              (mb_clk),
        .pll_clk             (rx_pll_clk), // Use delayed clock
        .i_rst_n             (rst_n),
