`timescale 1ns/1ps
// =============================================================================
// Testbench : unit_rx_deser_tb_s2
// Purpose   : Verification of Solution 2 (SR History Clear)
// =============================================================================
module unit_rx_deser_tb_s2;
    ...
    // RX: Valid Deserializer (s2: history clear)
    unit_valid_deserializer_s2 #(.DATA_WIDTH(DATA_WIDTH)) u_valid_des (
        .pll_clk     (rx_pll_clk), // Use delayed clock
        .i_rst_n     (rst_n),
        .ser_data_in (valid_serial_out),
        .i_clear     (valid_frame_pulse), // FEEDBACK LOOP
        .o_shift_reg (valid_shift_reg)
    );

    // RX: Valid Frame Detector
    ...
    unit_data_deserializer_s2 #(.DATA_WIDTH(DATA_WIDTH)) u_data_des (
        .mb_clk              (mb_clk),
        .pll_clk             (rx_pll_clk), // Use delayed clock
        .i_rst_n             (rst_n),
    ...
    initial begin
        $dumpfile("rx_deser_tb_s2.vcd");
        $dumpvars(0, unit_rx_deser_tb_s2);
        ...
        $display("");
        $display("==========================================================");
        $display("  RX Deserializer Solution 2 Testbench");
