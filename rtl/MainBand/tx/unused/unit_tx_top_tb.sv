...
//                unit_valid_tx and the mb_clk (slow) side of every serializer.
...
//        unit_clkdiv holds lclk static while i_rst_n=0, so reset is sequenced off
//        dut.pll_clk, not lclk.
//
//        Unlike MB_TX_TOP, unit_tx_top does NOT bring the PLL clock / period out to
...
//    * unit_valid_tx has no IDLE state and resets into VALID_FRAME. To exercise the
...
//    * unit_valid_tx drives a fixed TVLD word (0x0F0F0F0F) and gates the valid lane
//      from the LFSR serializer-enable (ser_en_lfsr_i), not from valid_frame_en.
//    * unit_clk_pattern_gen_tx has no i_period port; TCKN_P is TCKP_P delayed by a
...
module unit_tx_top_tb;
    ...
    // LFSR state codes (matching unit_lfsr_tx localparams)
    ...
    logic                     i_rst_n;
    logic                     lclk_g;   // unit_clk_gate enable - MUST be high to ungate
    ...
    // =========================================================================
    // Helpers  (pll_clk tapped hierarchically – unit_tx_top keeps it internal)
    // =========================================================================
    ...
    // unit_mapper reference for WIDTH_DEG_ALL (DEGRADE_LANES_0_TO_15), no reversal.
    ...
    initial begin
        ...
        //         unit_clkdiv holds lclk static while i_rst_n=0.
        // ------------------------------------------------------------------
        ...
        // ------------------------------------------------------------------
        // Step 2: release reset on a pll_clk edge, then wait for unit_clkdiv to
        ...
        @(negedge lclk);
        i_valid_pattern_en = 1'b1;     // HELD high (unsued unit_valid_tx needs this)
        ...
        begin
            ...
            if (err_map == 0)
                $display("  [OK]      MAPPING : all 16 unit_mapper lanes match the lp_data slicing.");
            else
        ...
        // ==================================================================
        $display("\n=================================================");
        $display("  unit_tx_top testbench COMPLETE - all phases done.");
        $display("=================================================\n");
        $stop;
    end
