...
module mb_die #(
    ...
    //                     deserialiser output. Pipeline stages:
    //                       1 MB_clk serialisation pipeline (Serializer latency)
    //                       3 MB_clk CDC + deserialiser latency (3-FF sync + deser)
    //                     Total = 4 MB_clk cycles.
    //   RX_VALID_DELAY  : Additional delay on rx_data_valid so the Demapper does
    //                     not consume data before LFSR_RX and PATTERN_COMPARATOR
    //                     have fully settled. The LFSR_RX registered output
    //                     (o_Data_by) is ready at T0+5, which is when the
    //                     Demapper must see rx_data_valid=1.
    //                     Total = 5 MB_clk cycles.
    // =========================================================================
    localparam int RX_ACTIVE_DELAY = 4;
    ...
    // When i_reversal_en=1, reverse the bit order of the received data lanes
    // so that lane N of the partner's TX drives lane (NUM_LANES-1-N) here.
    // The testbench also reverses the channel (d1_to_d0_data[i] = d1_TD_P[15-i]),
    // so the two reversals cancel and the original lane order is restored.
    wire [15:0] rx_data_swapped;
    genvar rev_i;
    generate
        for (rev_i = 0; rev_i < NUM_LANES; rev_i = rev_i + 1) begin : gen_rev
            assign rx_data_swapped[rev_i] = i_reversal_en
                ? i_RD_P[NUM_LANES-1-rev_i]
                : i_RD_P[rev_i];
        end
    endgenerate
