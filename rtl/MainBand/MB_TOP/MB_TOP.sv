...
module MB_TOP #(
    ...
    // ─────────────────────────────────────────────────────────────────────
    // RX always uses the functional MB clock — no mux needed

    localparam int RX_ACTIVE_DELAY = 3; // 0-indexed, so 3 means 4 cycles delay
    localparam int RX_VALID_DELAY  = 2; // 0-indexed, so 2 means 3 cycles delay

    logic [7:0] active_state_delay_reg;
    always_ff @(posedge o_mb_clk or negedge i_rst_n) begin
        if (!i_rst_n) begin
            active_state_delay_reg <= 8'd0;
        end else begin
            active_state_delay_reg <= {active_state_delay_reg[6:0], i_active_state_entered};
        end
    end
    logic rx_active_state_entered_gate;
    assign rx_active_state_entered_gate = active_state_delay_reg[RX_ACTIVE_DELAY];

    logic [2:0] rx_data_valid_delay_reg;
    always_ff @(posedge o_mb_clk or negedge i_rst_n) begin
        if (!i_rst_n) begin
            rx_data_valid_delay_reg <= 3'd0;
        end else begin
            rx_data_valid_delay_reg <= {rx_data_valid_delay_reg[1:0], rx_data_valid};
        end
    end
    logic rx_data_valid_gated;
    assign rx_data_valid_gated = (i_descramble_en) ? rx_data_valid_delay_reg[RX_VALID_DELAY] : rx_data_valid;
    ...
    MB_RX_TOP #(
        ...
        .i_active_state_entered          (rx_active_state_entered_gate),
