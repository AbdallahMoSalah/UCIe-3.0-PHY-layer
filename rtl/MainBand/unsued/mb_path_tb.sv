// =============================================================================
// Testbench : mb_path_tb
// Purpose   : Integration check of the MainBand datapath
//
//     Mapper -> LFSR_TX -> MB_SERIALIZER x16 -> MB_DESERIALIZER x16
//            -> LFSR_RX -> Demapper
//
...
//   LFSR_RX must equal the Mapper's lane words, and the Demapper output must
...
//
//   Checks:
//     A) LFSR_RX o_Data_by[i] == Mapper o_lane[i]   (per-lane, 16 lanes)
//     B) Demapper o_out_data  == demapper_model(mapper lanes)
// =============================================================================
`timescale 1ns/1ps
...
module mb_path_tb;
    ...
    logic [W-1:0] map_lane [0:15];     // Mapper outputs (TX lanes, golden)
    logic [W-1:0] tx_scr   [0:15];     // LFSR_TX scrambled outputs
    logic [15:0]  ser_out;             // 16 serial lines
    logic [W-1:0] des_word [0:15];     // Deserializer parallel outputs
    logic         de_done  [0:15];
    logic [W-1:0] rx_lane  [0:15];     // LFSR_RX descrambled outputs (RX lanes)
    ...
    // Stage 1: Mapper
    // =========================================================================
    Mapper #(.WIDTH(W), .NUM_LANES(16), .N_BYTES(N_BYTES)) u_map (
    ...
    generate
        for (g = 0; g < 16; g = g + 1) begin : g_lane
            ...
            MB_DESERIALIZER #(.DATA_WIDTH(W)) u_des (
    ...
    // Stage 6: Demapper
    // =========================================================================
    Demapper #(.N_BYTES(N_BYTES), .NUM_LANES(16), .WIDTH(W)) u_demap (
        .i_clk(MB_clk), .i_rst_n(i_rst_n),
    ...
    // =========================================================================
    // Golden model of the x16 demapper (from captured mapper lanes)
    // =========================================================================
    ...
    initial begin
        ...
        $display("\n--- CHECK A: LFSR_RX lanes vs Mapper lanes ---");
        ...
        $display("\n--- CHECK B: Demapper output vs model ---");
        $display("  out=0x%032h", out_data_cap);
        $display("  exp=0x%032h", exp_out);
        if (out_data_cap !== exp_out) begin
            fails=fails+1;
            $display("  Demapper MISMATCH");
        end else $display("  Demapper MATCH  (pl_valid captured)");
