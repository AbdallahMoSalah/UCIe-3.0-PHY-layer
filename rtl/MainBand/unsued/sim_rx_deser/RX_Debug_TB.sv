...
module RX_Debug_TB;
    ...
    Valid_Deserializer #(.DATA_WIDTH(DATA_WIDTH)) u_data_des (
        .pll_clk(pll_clk), .i_rst_n(rst_n),
    ...
    // Also try: use the existing MB_DESERIALIZER with counter to see what it gets
    wire [DATA_WIDTH-1:0] ref_valid_out, ref_data_out;
    wire ref_valid_done, ref_data_done;

    MB_DESERIALIZER #(.DATA_WIDTH(DATA_WIDTH)) u_ref_valid (
        .MB_clk(mb_clk), .pll_clk(pll_clk), .i_rst_n(rst_n),
    ...
    MB_DESERIALIZER #(.DATA_WIDTH(DATA_WIDTH)) u_ref_data (
        .MB_clk(mb_clk), .pll_clk(pll_clk), .i_rst_n(rst_n),
