...
//   stream to the mapper word stream. A misaligned LFSR yields per-word-varying
...
module mb_path_stream_tb;
    ...
    logic [W-1:0] map_q [0:NCAP-1][0:15];   // mapper words actually emitted
    ...
    always @(posedge MB_clk) if (dbg_trace)
        $display("    TRACE tx_scr0=%08h ser_en=%b | des0=%08h de=%b | rxact=%b rxdsc=%b rx0=%08h",
                 tx_scr[0], ser_en_w, des_word[0], de_done[0], rx_active, rx_descr, rx_lane[0]);

    // capture mapper output words while scrambling is active
    ...
    task automatic run_stream(input [127:0] name, input [2:0] m, input int nf,
        ...
        begin
            ...
            // ---- start TX scrambling + mapper streaming ----
            @(negedge MB_clk);
            ...
            fork
                ...
                begin : rx_start
                    // assert RX descramble rx_lead cycles after tx_active
