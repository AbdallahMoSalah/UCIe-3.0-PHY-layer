...
//
//   Mapper -> LFSR_TX -> MB_SERIALIZER x16 ----------------\
//                                                           >-- MB_DESERIALIZER_FRAMED x16
...
module mb_framed_path_tb;
    ...
    wire          valid_ser_en;     // VALID_TX serializer-enable (= ser_en_w in VALID_FRAME)
    wire          valid_done;
    ...
    MB_SERIALIZER #(.DATA_WIDTH(W)) u_valid_ser (
    ...
    generate
        for (g=0; g<16; g=g+1) begin : g_lane
            ...
            MB_DESERIALIZER_FRAMED #(.DATA_WIDTH(W)) u_des (
                .MB_clk(MB_clk), .pll_clk(pll_rx), .i_rst_n(i_rst_n),
    ...
    task automatic run_stream(input [127:0] name, input [2:0] m, input int nf,
        ...
        begin
            ...
            // ---- start TX scrambling + mapper streaming; hold des_en high ----
            @(negedge MB_clk);
