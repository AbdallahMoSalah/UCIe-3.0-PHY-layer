...
(
    ...
    typedef enum logic [2:0] {
        ...
        IQ_MEASURE          = 3'h1,
        IQ_EVAL             = 3'h2,
        IQ_SEND_SHIFT_REQ   = 3'h3,
        IQ_WAIT_RESP        = 3'h4,
        IQ_SETTLE           = 3'h5,
        IQ_DONE_SUCCESS     = 3'h6,
        IQ_DONE_ERROR       = 3'h7
    } state_t;
    ...
    always_comb begin
        ...
        case (current_state)
            IQ_IDLE: begin
                if (iq_en) next_state = IQ_MEASURE;
                else       next_state = IQ_IDLE;
            end

            IQ_MEASURE: begin
                if (analog_settle_time_done) next_state = IQ_EVAL;
                else                         next_state = IQ_MEASURE;
            end

            IQ_EVAL: begin
                if (phy_rx_tckn_shift == 5'd0)
                    next_state = IQ_DONE_SUCCESS;
                else
                    next_state = IQ_SEND_SHIFT_REQ;
            end

            IQ_SEND_SHIFT_REQ: begin
                // Transition unconditionally to WAIT_RESP on the next cycle,
                // ensuring the output message valid pulse lasts exactly 1 cycle.
                next_state = IQ_WAIT_RESP;
            end

            IQ_WAIT_RESP: begin
                if (rx_sb_msg_valid && rx_sb_msg == MBTRAIN_RXCLKCAL_TCKN_L_shift_resp) begin
                    if (rx_msginfo[0] == 1'b1) // Out of Range status
                        next_state = IQ_DONE_ERROR;
                    else
                        next_state = IQ_SETTLE;
                end
                else begin
                    next_state = IQ_WAIT_RESP;
                end
            ...
            IQ_SETTLE: begin
                if (analog_settle_time_done) next_state = IQ_MEASURE;
                else                         next_state = IQ_SETTLE;
            end

            IQ_DONE_SUCCESS: begin
                if (!iq_en) next_state = IQ_IDLE;
                else        next_state = IQ_DONE_SUCCESS;
            end

            IQ_DONE_ERROR: begin
                if (!iq_en) next_state = IQ_IDLE;
                else        next_state = IQ_DONE_ERROR;
            end

            default: next_state = IQ_IDLE;
        endcase
    end
