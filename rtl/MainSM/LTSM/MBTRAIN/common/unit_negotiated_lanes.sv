...
module unit_negotiated_lanes (
    ...
    //==========================================================================//
    input  logic [15:0] success_lanes,             // A 16-bit vector where each bit represents the status of the corresponding physical data lane.
    ...
    output logic        degrade_feasible           // Feasibility flag for width degradation:
                                                   // 1 = A valid degraded lane map configuration was successfully resolved (degraded_lane_map_code != 3'b000);
                                                   // 0 = No valid degradation mapping is feasible (degraded_lane_map_code == 3'b000).
);
    ...
    always_comb begin : RX_DECODE
        case (mb_rx_data_lane_mask)
            3'b001 : active_rx_lanes = 16'h00FF; // Lanes 0–7   (x8 low half)
            3'b010 : active_rx_lanes = 16'hFF00; // Lanes 8–15  (x8 high half)
            3'b011 : active_rx_lanes = 16'hFFFF; // Lanes 0–15  (x16 full width)
            3'b100 : active_rx_lanes = 16'h000F; // Lanes 0–3   (x4 low quarter)
            3'b101 : active_rx_lanes = 16'h00F0; // Lanes 4–7   (x4 mid-low quarter)
    ...
    always_comb begin : TX_DECODE
        case (mb_tx_data_lane_mask)
            3'b001 : active_tx_lanes = 16'h00FF; // Lanes 0–7   (x8 low half)
            3'b010 : active_tx_lanes = 16'hFF00; // Lanes 8–15  (x8 high half)
            3'b011 : active_tx_lanes = 16'hFFFF; // Lanes 0–15  (x16 full width)
            3'b100 : active_tx_lanes = 16'h000F; // Lanes 0–3   (x4 low quarter)
            3'b101 : active_tx_lanes = 16'h00F0; // Lanes 4–7   (x4 mid-low quarter)
    ...
    always_comb begin : DEGRADE_EVAL
        if ((rf_cap_SPMW == 1'b0 && rf_ctrl_target_link_width == 4'h2) && param_UCIe_S_x8 == 1'b0) begin
            ...
            if (success_lanes[7:0] == 8'hFF)
                degraded_lane_map_code = 3'b001; // Logical Lanes 0 to 7
            else if (success_lanes[3:0] == 4'hF)
                degraded_lane_map_code = 3'b100; // Logical Lanes 0 to 3
            else if (success_lanes[7:4] == 4'hF)
