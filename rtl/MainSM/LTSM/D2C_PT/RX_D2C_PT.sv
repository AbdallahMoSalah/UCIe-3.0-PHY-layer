...
module RX_D2C_PT  #() (
    ...
    //=====================================//
    // Control Signals for Sub-states:  v  //
    //=====================================//
    ...
    always @(*) begin
        if(d2c_if.timeout_8ms_occured | (sb_if.rx_sb_msg == TRAINERROR_Entry_req && sb_if.rx_sb_msg_valid == 1'b1)) begin
            // (S11)
            ...
            case (current_state)
                ...
                RX_PT_IDLE: begin
                    if (d2c_if.rx_pt_en) next_state = RX_PT_START_REQ;
                    else next_state = RX_PT_IDLE;
                end
                // (S1) Send & Receive SB Message: {Start Rx Init D to C point test req}
                RX_PT_START_REQ: begin
                    if (sb_if.rx_sb_msg == Start_Rx_Init_D_to_C_point_test_req && sb_if.rx_sb_msg_valid == 1'b1) next_state = RX_PT_START_RESP;
                    else next_state = RX_PT_START_REQ;
                end
                // (S2) Send & Receive SB Message: {Start Rx Init D to C point test resp}.
                RX_PT_START_RESP: begin
                    if (sb_if.rx_sb_msg == Start_Rx_Init_D_to_C_point_test_resp && sb_if.rx_sb_msg_valid == 1'b1) next_state = RX_PT_CLR_ERR_REQ;
                    else next_state = RX_PT_START_RESP;
                end
                // (S3) Send & Receive SB Message: {LFSR clear error req}.
                RX_PT_CLR_ERR_REQ: begin
                    if (sb_if.rx_sb_msg == LFSR_clear_error_req && sb_if.rx_sb_msg_valid == 1'b1) next_state = RX_PT_CLR_ERR_RESP;
                    else next_state = RX_PT_CLR_ERR_REQ;
                end
                // (S4) Send & Receive SB Message: {LFSR clear error resp}.
                RX_PT_CLR_ERR_RESP: begin
                    if (sb_if.rx_sb_msg == LFSR_clear_error_resp && sb_if.rx_sb_msg_valid == 1'b1) next_state = RX_PT_PATTERN_GEN;
                    else next_state = RX_PT_CLR_ERR_RESP;
                end
                // (S5) Send & Receive MB Pattern
                RX_PT_PATTERN_GEN: begin
                    if (mb_if.mb_tx_pattern_count_done) next_state = RX_PT_COUNT_DONE_REQ;
                    else next_state = RX_PT_PATTERN_GEN;
                end
                // (S6) Send & Receive SB Message {Rx Init D to C Tx count done req}.
                RX_PT_COUNT_DONE_REQ: begin
                    if (sb_if.rx_sb_msg == Rx_Init_D_to_C_Tx_Count_Done_req && sb_if.rx_sb_msg_valid == 1'b1) next_state = RX_PT_COUNT_DONE_RESP;
                ...
                RX_PT_COUNT_DONE_RESP: begin
                    if (sb_if.rx_sb_msg == Rx_Init_D_to_C_Tx_Count_Done_resp && sb_if.rx_sb_msg_valid == 1'b1) next_state = RX_PT_END_REQ;
                    else next_state = RX_PT_COUNT_DONE_RESP;
                end
                // (S8) Send & Receive SB Message: {End Rx Init D to C point test req}.
                RX_PT_END_REQ: begin
                    if (sb_if.rx_sb_msg == End_Rx_Init_D_to_C_point_test_req && sb_if.rx_sb_msg_valid == 1'b1) next_state = RX_PT_END_RESP;
                    else next_state = RX_PT_END_REQ;
                end
                // (S9) Send & Receive SB Message: {End Rx Init D to C point test resp}.
                RX_PT_END_RESP: begin
                    if (sb_if.rx_sb_msg == End_Rx_Init_D_to_C_point_test_resp && sb_if.rx_sb_msg_valid == 1'b1) next_state = RX_PT_DONE;
                    else next_state = RX_PT_END_RESP;
                end
                // (S10)
                RX_PT_DONE: begin
                    next_state = (d2c_if.rx_pt_en)? RX_PT_DONE : RX_PT_IDLE; // Stay here for 1 lclk cycle.
                end
    ...
    always @(*) begin
        ...
        mb_if.mb_rx_max_err_thresh_perlane = rf_if.cfg_train4_max_err_thresh_perlane;  // Max error Threshold in per-Lane comparison for error counting. From "Training Setup 4 (Offset 1050h)" in RF.
        mb_if.mb_rx_max_err_thresh_aggr    = rf_if.cfg_train4_max_err_thresh_aggr   ;  // Max error Threshold in aggregate comparison for error counting. From "Training Setup 4 (Offset 1050h)" in RF. 
        mb_if.mb_rx_compare_setup          = d2c_if.d2c_compare_setup               ;  // 0: Per-Lane, 1: Aggregate, 2: Valid Lane, 3: Clock Lane Comparison.
        mb_if.mb_tx_iter_count             = d2c_if.d2c_iter_count                  ;  // Iteration Count: Indicates the iteration count of bursts followed by idle.
        mb_if.mb_tx_idle_count             = d2c_if.d2c_idle_count                  ;  // IDLE Count: Indicates the duration of low following the burst (UI count).
        mb_if.mb_tx_burst_count            = d2c_if.d2c_burst_count                 ;  // Burst Count: Indicates the duration of selected pattern (UI count).
        mb_if.mb_tx_pattern_mode           = d2c_if.d2c_pattern_mode                ;  // 0: Continuous Pattern Mode, 1: Burst Pattern Mode.
        mb_if.mb_tx_clk_sampling           = d2c_if.d2c_clk_sampling                ;  // Clock Phase control: 0h(Eye Center), 1h(Left edge), 2h(Right edge).
        mb_if.mb_tx_val_pattern_sel        = d2c_if.d2c_val_pattern_sel             ;  // 0: VALTRAIN pattern, 1: Don't use VALTRAIN, 2: Operational Valid.
        mb_if.mb_tx_data_pattern_sel       = d2c_if.d2c_data_pattern_sel            ;  // Data pattern used during training: 0h: LFSR; 1h: ID, or all 0.
        ...
        case (current_state)
            ...
            RX_PT_START_REQ: begin
                ...
                sb_if.tx_sb_msg            = Start_Rx_Init_D_to_C_point_test_req;
                ...
                sb_if.tx_data_field[9:6]   =  {2'b0, d2c_if.d2c_clk_sampling}    ; // Clock Phase control at Transmitter (0h: Clock PI Center, 1h: Left Edge, 2h: Right Edge).
                sb_if.tx_data_field[5:3]   =  {2'b0, d2c_if.d2c_val_pattern_sel} ; // Valid Pattern (0h: Functional pattern).
                sb_if.tx_data_field[2:0]   =  {1'b0, d2c_if.d2c_data_pattern_sel}; // Data pattern (0h: LFSR, 1h: Per Lane ID).
            ...
            RX_PT_START_RESP: begin
                ...
                sb_if.tx_sb_msg_valid     = (~data_incoherence); // Assert valid only when data incoherence flag is cleared, to avoid sending incorrect messages.
                sb_if.tx_sb_msg           = Start_Rx_Init_D_to_C_point_test_resp;
            ...
            RX_PT_CLR_ERR_REQ: begin
                ...
                sb_if.tx_sb_msg           = LFSR_clear_error_req;
                sb_if.tx_msginfo          = 16'b0;
            ...
            RX_PT_CLR_ERR_RESP: begin
                sb_if.tx_sb_msg_valid     = (~data_incoherence);
                sb_if.tx_sb_msg           = LFSR_clear_error_resp;
                sb_if.tx_msginfo          = 16'b0;
                sb_if.tx_data_field[63:0] = 64'b0; // No payload.
            ...
            RX_PT_COUNT_DONE_REQ: begin
                sb_if.tx_sb_msg_valid     = (~data_incoherence);
                sb_if.tx_sb_msg           = Rx_Init_D_to_C_Tx_Count_Done_req;
                sb_if.tx_msginfo          = 16'b0;
                sb_if.tx_data_field[63:0] = 64'b0; // No payload.
            ...
            RX_PT_COUNT_DONE_RESP: begin
                sb_if.tx_sb_msg_valid     = (~data_incoherence);
                sb_if.tx_sb_msg           = Rx_Init_D_to_C_Tx_Count_Done_resp;
                sb_if.tx_msginfo          = 16'b0;
                sb_if.tx_data_field[63:0] = 64'b0; // No payload.
            ...
            RX_PT_END_REQ: begin
                sb_if.tx_sb_msg_valid     = (~data_incoherence);
                sb_if.tx_sb_msg           = End_Rx_Init_D_to_C_point_test_req;
                sb_if.tx_msginfo          = 16'b0;
                sb_if.tx_data_field[63:0] = 64'b0; // No payload.
            end
            // (S9) Send & Receive SB Message: {End Rx Init D to C point test resp}.
            RX_PT_END_RESP: begin
                sb_if.tx_sb_msg_valid     = (~data_incoherence);
                sb_if.tx_sb_msg           = End_Rx_Init_D_to_C_point_test_resp;
                sb_if.tx_msginfo          = 16'b0;
                sb_if.tx_data_field[63:0] = 64'b0; // No payload.
            end
            // (S10)
            RX_PT_DONE: begin
                ...
                sb_if.tx_msginfo          = 16'b0;
                sb_if.tx_data_field[63:0] = 64'b0; // No payload.
                d2c_if.test_d2c_done      = 1'b1 ; // Assert the test done signal to tell the external Sub-state the completion of the test.
            end
            // (S11) TRAINERROR state:
            TO_TRAINERROR: begin
                d2c_if.test_d2c_done = 0;
