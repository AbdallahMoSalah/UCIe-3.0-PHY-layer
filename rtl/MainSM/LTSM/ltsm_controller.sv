...
(
    ...
    always_comb begin
        ...
        // Global transition to TRAINERROR on timeouts, explicit requests, or RDI status
        if ((state_req == TRAINERROR || trainerror_req || timeout_8ms_occured) && (current_state != CTRL_TRAINERROR)) begin
            next_state = CTRL_TRAINERROR;
        end else begin
            case (current_state)
                CTRL_RESET: begin
                    if (reset_done) next_state = CTRL_SBINIT;
                end

                CTRL_SBINIT: begin
                    if (sbinit_done) next_state = CTRL_MBINIT;
                end

                CTRL_MBINIT: begin
                    if (mbinit_error)       next_state = CTRL_TRAINERROR;
                    else if (mbinit_done)   next_state = CTRL_MBTRAIN;
                end

                CTRL_MBTRAIN: begin
                    if (mbtrain_error)       next_state = CTRL_TRAINERROR;
                    else if (phyretrain_req) next_state = CTRL_PHYRETRAIN;
                    else if (mbtrain_done)   next_state = CTRL_LINKINIT;
                end

                CTRL_LINKINIT: begin
                    if (linkinit_done) next_state = CTRL_ACTIVE;
                end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (next_state == CTRL_TRAINERROR && current_state != CTRL_TRAINERROR) begin
                if (timeout_8ms_occured) begin
                    log1_state_timeout_occ_reg       <= 1'b1;
                    log1_state_timeout_occ_valid_reg <= 1'b1;
                end else if (state_req == TRAINERROR || trainerror_req) begin
