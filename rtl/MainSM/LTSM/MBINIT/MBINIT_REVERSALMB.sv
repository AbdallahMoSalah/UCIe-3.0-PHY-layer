...
// RESULT
////////////////////////////////////////////////////////
logic [15:0] mb_rx_perlane_err_majority;
assign mb_rx_perlane_err_majority = mb_rx_perlane_err;
    ...
    if(!rst_n) begin
        ...
        MB_S4_RESULT_EXCHANGE_RSP:
            if(!s4_rsp_sent) begin
                ...
                mb_reversal_tx_MsgInfo <=mb_rx_perlane_err_majority;
                mb_reversal_tx_data_Field <= MB_default_data_Field;
                s4_rsp_sent <= 1;
            end
...
always_ff @(posedge clk or negedge rst_n) begin
    if(!rst_n)
        mb_reversal_error <= 0;
    else if(!majority_success && current_state == MB_S5_DECISION)
        mb_reversal_error <= 1;
end
