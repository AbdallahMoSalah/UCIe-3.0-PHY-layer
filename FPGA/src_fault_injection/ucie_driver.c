...
// =============================================================================
// RDI control (bits [7:0]) and fault control (bits [27:8]) share one EMIO bank.
// XGpioPs_Write drives the whole 32-bit bank, so every update must re-drive the
// union of both shadows or it would clobber the other set of outputs.
static void Ucie_Emio_Commit(UcieDriver *Dev) {
    XGpioPs_Write(&Dev->Gpio, UCIE_EMIO_BANK, Dev->CtrlShadow | Dev->FaultShadow);
}
...
int Ucie_Init(UcieDriver *Dev, UINTPTR DmaBaseAddr, UINTPTR FifoBaseAddr, UINTPTR GpioBaseAddr) {
    ...
    // All PS-driven bits live on one EMIO bank: RDI control [7:0] and the
    // fault-injection controls [27:8]. Set direction/OE once for the union; the
    // remaining bits stay inputs (status / pl_*, read from the input register).
    XGpioPs_SetDirection(&Dev->Gpio, UCIE_EMIO_BANK, UCIE_EMIO_OUT_MASK);
    XGpioPs_SetOutputEnable(&Dev->Gpio, UCIE_EMIO_BANK, UCIE_EMIO_OUT_MASK);
    ...
    // ---- 4. Fault-injection outputs (same EMIO bank) : arm idle + commit ----
    Ucie_Fi_Init(Dev);
...
void Ucie_Rdi_WriteCtrl(UcieDriver *Dev, u32 CtrlBits) {
    ...
    Ucie_Emio_Commit(Dev);
}

// =============================================================================
// Fault-injection controls. Bits [27:8] of the shared EMIO bank are PS outputs
// wired to the injector's {rtl_reset_n, block_sideband, vld_err, reverse,
// corrupt[15:0]}. Direction/OE for the bank is set once in Ucie_Init().
// =============================================================================
void Ucie_Fi_Init(UcieDriver *Dev) {
    // Idle: faults off, RTL reset de-asserted (bit 27 = 1 -> running).
    Dev->FaultShadow = FI_RTL_RESET_N_MASK;
    Ucie_Emio_Commit(Dev);
}

static void Ucie_Fi_Write(UcieDriver *Dev, u32 Bits) {
    Dev->FaultShadow = Bits & UCIE_FI_CTRL_MASK;
    Ucie_Emio_Commit(Dev);
}

void Ucie_Fi_Clear(UcieDriver *Dev) {
    // Faults off, but keep the RTL running (do NOT re-assert reset).
    Ucie_Fi_Write(Dev, FI_RTL_RESET_N_MASK);
}

void Ucie_Fi_Set(UcieDriver *Dev, u16 CorruptMask, int Reverse, int VldErr, int BlockSideband) {
    u32 b = ((u32)CorruptMask << FI_CORRUPT_SHIFT) & FI_CORRUPT_MASK;
    if (Reverse)        b |= FI_REVERSE_MASK;
    if (VldErr)         b |= FI_VLD_ERR_MASK;
    if (BlockSideband)  b |= FI_BLOCK_SIDEBAND_MASK;
    b |= FI_RTL_RESET_N_MASK;   // keep the RTL out of reset while faults are applied
    Ucie_Fi_Write(Dev, b);
}

// Hardware RTL reset via EMIO bit 27 (active-low): drive it low to assert reset,
// hold HoldUs microseconds, then drive it high to release. Fault bits are left
// as-is; the shared-bank commit re-drives RDI control alongside.
void Ucie_Fi_ResetRtl(UcieDriver *Dev, u32 HoldUs) {
    Dev->FaultShadow &= ~FI_RTL_RESET_N_MASK;   // bit 27 = 0 -> hold RTL in reset
    Ucie_Emio_Commit(Dev);
    usleep(HoldUs);
    Dev->FaultShadow |= FI_RTL_RESET_N_MASK;     // bit 27 = 1 -> release
    Ucie_Emio_Commit(Dev);
    usleep(HoldUs);                              // let the RTL come out of reset
}

void Ucie_Fi_SetCorrupt(UcieDriver *Dev, u16 CorruptMask) {
    u32 b = (Dev->FaultShadow & ~FI_CORRUPT_MASK)
          | (((u32)CorruptMask << FI_CORRUPT_SHIFT) & FI_CORRUPT_MASK);
    Ucie_Fi_Write(Dev, b);
}

void Ucie_Fi_SetReverse(UcieDriver *Dev, int on) {
    Ucie_Fi_Write(Dev, on ? (Dev->FaultShadow | FI_REVERSE_MASK)
                          : (Dev->FaultShadow & ~FI_REVERSE_MASK));
}

void Ucie_Fi_SetVldErr(UcieDriver *Dev, int on) {
    Ucie_Fi_Write(Dev, on ? (Dev->FaultShadow | FI_VLD_ERR_MASK)
                          : (Dev->FaultShadow & ~FI_VLD_ERR_MASK));
}

void Ucie_Fi_SetBlockSideband(UcieDriver *Dev, int on) {
    Ucie_Fi_Write(Dev, on ? (Dev->FaultShadow | FI_BLOCK_SIDEBAND_MASK)
                          : (Dev->FaultShadow & ~FI_BLOCK_SIDEBAND_MASK));
}
...
// =============================================================================
// Start training with EXPLICIT targets (mirrors the TB program_die_custom):
// program TRAIN_SETUP3/4 + PHY_CONTROL(+ForceX8) + LINK_CTRL(width,speed,start).
// Width/Speed are the raw 4-bit codes from spec Table 9-9 (see ucie_config.h).
// =============================================================================
int Ucie_StartTrainingCustom(UcieDriver *Dev, u8 TargetWidth, u8 TargetSpeed, int ForceX8) {
    int s3, s4, sphy, slink;
    ...
    u64 phy_control_val = CFG_PHY_CONTROL_BASE;
    if (ForceX8) phy_control_val |=  PHY_CTRL_FORCE_X8;
    else         phy_control_val &= ~(u64)PHY_CTRL_FORCE_X8;

    u64 link_ctrl_val = 0ULL;
    link_ctrl_val |= ((u64)TargetWidth << LINK_CTRL_TGT_WIDTH_SHIFT) & LINK_CTRL_TGT_WIDTH_MASK;
    link_ctrl_val |= ((u64)TargetSpeed << LINK_CTRL_TGT_SPEED_SHIFT) & LINK_CTRL_TGT_SPEED_MASK;
    link_ctrl_val |= LINK_CTRL_START_TRAINING;
    ...
    xil_printf("    reg-write (custom w=0x%X s=0x%X x8=%d): SETUP3=%d SETUP4=%d "
               "PHY_CTRL=%d LINK_CTRL=%d\r\n",
               TargetWidth, TargetSpeed, ForceX8, s3, s4, sphy, slink);
...
// =============================================================================
// Best-effort soft link reset for running scenarios back-to-back.
//   Drives lp_state_req = RESET, services the handshake until pl_state_sts shows
//   RESET (or times out), then returns lp_state_req to NOP so the next
//   StartTraining/BringUpActive sequence starts from a clean request.
//
//   NOTE: This does NOT toggle the PHY's rst_n (that is not GPIO-mapped). It
//   relies on the RDI RESET path + a fresh start-training write to re-arm the
//   link. For fully independent scenarios, pulse the PL reset (PS reset / a
//   dedicated GPIO) between runs; hook that in here if your design exposes it.
// =============================================================================
int Ucie_LinkResetSoft(UcieDriver *Dev, u32 TimeoutMs) {
    u32 deadline = TimeoutMs * 1000;

    Ucie_Rdi_SetStateReq(Dev, RDI_STATE_RESET);
    Ucie_Dbg_Mark(Dev, "req-RST");

    while (deadline--) {
        u32 st = Ucie_Rdi_ReadStatus(Dev);
        u32 state = (st & RDI_STAT_PL_STATE_STS_MASK) >> RDI_STAT_PL_STATE_STS_SHIFT;
        if (state == RDI_STATE_RESET) {
            Ucie_Dbg_Mark(Dev, "in-RST");
            break;
        }
        Ucie_Rdi_ServiceHandshake(Dev, st);
        Ucie_Dbg_Poll(Dev, "rst-wait");
        usleep(1);
    }

    // Return the request line to NOP and drop any lingering clk/stall acks so the
    // next bring-up starts from idle.
    Ucie_Rdi_SetStateReq(Dev, RDI_STATE_NOP);
    Ucie_Rdi_WriteCtrl(Dev, Dev->CtrlShadow &
                       ~(RDI_CTRL_LP_CLK_ACK_MASK | RDI_CTRL_LP_STALLACK_MASK));

    // Drain any stale sideband completions so the next scenario starts clean.
    Ucie_Sb_FlushRx(Dev);
    return XST_SUCCESS;
}

// =============================================================================
// Drain all pending data from the sideband RX FIFO. After a scenario that times
// out or faults, the PHY may eventually push completions that nobody consumed.
// Leaving them in the FIFO causes the next Ucie_Sb_WriteReg / ReadReg to read
// stale packets, corrupting the handshake and potentially hanging the run.
// =============================================================================
void Ucie_Sb_FlushRx(UcieDriver *Dev) {
    int safety = 256;   // bound iterations in case occupancy is stuck
    while (XLlFifo_RxOccupancy(&Dev->AxiFifo) && --safety > 0) {
        u32 len = XLlFifo_RxGetLen(&Dev->AxiFifo);
        if (len == 0) break;
