...
// =============================================================================
// FAULT-INJECTION EMIO map  (only present in the fault-injection bitstream:
// top = UCIe_FPGA_top_wrapper_fi, or UCIe_FPGA_2die_wrapper).
//
//   The injector's fault controls share the SAME EMIO output bus as the RDI
//   lp_* controls (bank 3): the block-design slices tap one emio_gpio_o[29:0]
//   bus. The fault bits are appended just above the RDI control bits [7:0]:
//
//     emio_o bit [23:8]  -> corrupt[15:0]     (per-lane stuck-at-0)
//     emio_o bit [24]    -> reverse           (MB lane-order reversal)
//     emio_o bit [25]    -> vld_err           (flip MB valid-frame strobe)
//     emio_o bit [26]    -> block_sideband    (cut the sideband fold)
//     emio_o bit [27]    -> rtl_reset_n       (ACTIVE-LOW: 0 = hold RTL in reset)
//
//   Because RDI control and fault control live on ONE bank, every write drives
//   the whole 32-bit bank. The driver keeps CtrlShadow (bits [7:0]) and
//   FaultShadow (bits [27:8]) and always commits their OR (Ucie_Emio_Commit).
//
//   For the two-die top (UCIe_FPGA_2die_wrapper) the injector is bidirectional
//   (corrupt_0to1 / corrupt_1to0, ...). This driver targets the SINGLE-die
//   self-loopback FI top, where both TB directions fold onto one physical loop,
//   so corrupt/reverse/vld_err are single controls.
// =============================================================================
#define UCIE_FI_EMIO_BANK          UCIE_EMIO_BANK  // same bank as RDI (bank 3)

#define FI_CORRUPT_MASK            0x00FFFF00u   // bits [23:8]  per-lane corrupt
#define FI_CORRUPT_SHIFT           8
#define FI_RTL_RESET_N_MASK        0x01000000u   // bit 24  (active-low RTL reset)
#define FI_REVERSE_MASK            0x02000000u   // bit 25
#define FI_VLD_ERR_MASK            0x04000000u   // bit 26
#define FI_BLOCK_SIDEBAND_MASK     0x08000000u   // bit 27
#define UCIE_FI_CTRL_MASK          0x0FFFFF00u   // bits [27:8] are PS outputs

// All PS-driven output bits on the shared EMIO bank (RDI control | fault control).
#define UCIE_EMIO_OUT_MASK         (UCIE_EMIO_CTRL_MASK | UCIE_FI_CTRL_MASK)
...
typedef struct {
    XAxiDma  AxiDma;
    XLlFifo  AxiFifo;
    XGpioPs  Gpio;       // PS EMIO GPIO (bank 3: RDI ctrl [7:0] + fault ctrl [27:8])
    u32      CtrlShadow; // last RDI control bits [7:0] driven to the EMIO bank
    u32      FaultShadow;// last fault-injection bits [27:8] driven to the EMIO bank
...
// Program LINK_CTRL / PHY_CONTROL / TRAIN_SETUP4 with explicit targets (mirrors
// the TB program_die_custom) then assert start-training. Pass a full 4-bit
// width/speed code (Table 9-9); ForceX8 != 0 sets PHY_CONTROL[8].
int Ucie_StartTrainingCustom(UcieDriver *Dev, u8 TargetWidth, u8 TargetSpeed, int ForceX8);

// Drive the link back to RESET and re-arm training (best-effort, for running
// several scenarios back-to-back without a full PL reset). Faults already set
// stay applied across the retrain. NOTE: the scenario loop now uses the harder
// Ucie_Fi_ResetRtl() hardware reset instead; this remains for softer re-arms.
int Ucie_LinkResetSoft(UcieDriver *Dev, u32 TimeoutMs);

// Drain any stale completions left in the sideband RX FIFO (call between
// scenarios so leftover data from timed-out operations can't confuse the next).
...
// =============================================================================
// Fault-injection controls (shared RDI/FI EMIO bank). Ucie_Fi_Init() is called
// for you by Ucie_Init(); the setters compose the fault bits [27:8] from a
// shadow and re-drive the whole bank in one write. corrupt is a 16-bit per-lane
// stuck-at-0 mask (bit i -> lane i).
// =============================================================================
void Ucie_Fi_Init(UcieDriver *Dev);                       // arm idle (faults off, RTL running)
void Ucie_Fi_Clear(UcieDriver *Dev);                      // all faults off (RTL kept running)
...
// Hardware RTL reset via EMIO bit 27 (active-low): assert, hold HoldUs us, release.
