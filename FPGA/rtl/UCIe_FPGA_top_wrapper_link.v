`timescale 1ns/1ps
// =============================================================================
// Module  : UCIe_FPGA_top_wrapper_link
// Project : UCIe 3.0 PHY - FPGA bring-up
//
// Purpose : Non-loopback sibling of UCIe_FPGA_top_wrapper.  Integrates the four
//           AXI-Stream bridges with the UCIe_FPGA_link core (digital PHY, no
//           analog), but the MainBand/SideBand parallel PHY boundary is EXPOSED
//           on top-level ports instead of self-looped.
//
//           Build a die-to-die link by instantiating two of these and
//           cross-wiring the boundary:
//             dieA.o_mb_tx_*  <-> dieB.i_mb_rx_*     dieA.o_sb_tx_* <-> dieB.i_sb_rx_*
//             dieB.o_mb_tx_*  <-> dieA.i_mb_rx_*     dieB.o_sb_tx_* <-> dieA.i_sb_rx_*
//             i_sb_tx_rdy = 1'b1 on both dies (lossless parallel channel).
//
...
module UCIe_FPGA_top_wrapper_link #(
    ...
    // ---- Exposed MainBand parallel PHY boundary (die-to-die) ----
    output                                   o_mb_tx_ser_en,
    output [NUM_LANES*DATA_WIDTH_MB-1:0]     o_mb_tx_lane,
    output                                   o_mb_tx_valid_ser_en,
    output [DATA_WIDTH_MB-1:0]               o_mb_tx_valid_word,
    output                                   o_mb_tx_ckp,
    output                                   o_mb_tx_ckn,
    output                                   o_mb_tx_trk,
    input                                    i_mb_rx_data_valid,
    input  [NUM_LANES*DATA_WIDTH_MB-1:0]     i_mb_rx_lane,
    input                                    i_mb_rx_valid_frame_vld,
    input  [DATA_WIDTH_MB-1:0]               i_mb_rx_valid_frame_data,
    input                                    i_mb_rx_ckp,
    input                                    i_mb_rx_ckn,
    input                                    i_mb_rx_trk,

    // ---- Exposed SideBand parallel PHY boundary (die-to-die) ----
    output [63:0]                            o_sb_tx_data,
    output                                   o_sb_tx_vld,
    input                                    i_sb_tx_rdy,
    input  [63:0]                            i_sb_rx_data,
    input                                    i_sb_rx_vld,
    ...
    // =========================================================================
    // Intermediate Core Interconnect Nets (bridge <-> core adapter face)
    // =========================================================================
    ...
    // =========================================================================
    // UCIe Core with parallel PHY boundary exposed (FPGA Target, die-to-die)
    // =========================================================================
    UCIe_FPGA_link #(
