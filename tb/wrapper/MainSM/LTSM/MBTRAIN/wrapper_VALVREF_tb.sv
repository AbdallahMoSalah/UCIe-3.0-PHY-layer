...
module wrapper_VALVREF_tb;
    ...
    localparam int VREF_W = $clog2(MAX_VAL_VREF_CODE + 1);

    wire [VREF_W-1:0] dut_swept_code_sliced;
    wire [VREF_W-1:0] dut_best_code_sliced [0:15];
    assign dut_swept_code_sliced = dut_if.swept_code[VREF_W-1:0];
    for (genvar i = 0; i < 16; i++) begin
        assign dut_best_code_sliced[i] = dut_if.best_code[i][VREF_W-1:0];
    end

    wire [VREF_W-1:0] ptn_swept_code_sliced;
    wire [VREF_W-1:0] ptn_best_code_sliced [0:15];
    assign ptn_swept_code_sliced = ptn_if.swept_code[VREF_W-1:0];
    for (genvar i = 0; i < 16; i++) begin
        assign ptn_best_code_sliced[i] = ptn_if.best_code[i][VREF_W-1:0];
    end
    ...
    always @(posedge lclk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Insert new inputs (X Value Fix: drive 0 when valid is 0)
            dut2ptn_msg_sr[0]  <= dut_if.tb_muxed_tx_sb_msg_valid ? dut_if.tb_muxed_tx_sb_msg     : 8'h0;
            dut2ptn_info_sr[0] <= dut_if.tb_muxed_tx_sb_msg_valid ? dut_if.tb_muxed_tx_msginfo    : 16'h0;
            dut2ptn_data_sr[0] <= dut_if.tb_muxed_tx_sb_msg_valid ? dut_if.tb_muxed_tx_data_field : 64'h0;
            ...
            if (tb_ptn_inject_valid) begin
                ...
                ptn2dut_msg_sr[0]  <= ptn_if.tb_muxed_tx_sb_msg_valid ? ptn_if.tb_muxed_tx_sb_msg     : 8'h0;
                ptn2dut_info_sr[0] <= ptn_if.tb_muxed_tx_sb_msg_valid ? ptn_if.tb_muxed_tx_msginfo    : 16'h0;
                ptn2dut_data_sr[0] <= ptn_if.tb_muxed_tx_sb_msg_valid ? ptn_if.tb_muxed_tx_data_field : 64'h0;
            end
        end
    end
    ...
    // =========================================================================
    // Dynamic Vref Sweeping Eye Simulation
    // =========================================================================
        ...
        if (dut_if.sweep_en) begin
            ...
            if (code >= dut_eye_start && code <= dut_eye_end) begin
                if (assume_holes_after_quarter_eye_start && (code == dut_eye_start + (dut_eye_end - dut_eye_start)/4)) begin
                    dut_if.tb_force_val_pass <= 1'b0; // Mock fail (hole)
                end else begin
        ...
        if (ptn_if.sweep_en) begin
            ...
            if (code >= ptn_eye_start && code <= ptn_eye_end) begin
                if (assume_holes_after_quarter_eye_start && (code == ptn_eye_start + (ptn_eye_end - ptn_eye_start)/4)) begin
                    ptn_if.tb_force_val_pass <= 1'b0; // Mock fail (hole)
                end else begin
    ...
    // =========================================================================
    logic        dut_local_valvref_en = 0;
    logic        dut_local_valvref_done;
    ...
    // =========================================================================
    logic        ptn_local_valvref_en = 0;
    logic        ptn_local_valvref_done;
    ...
    assign dut_local_state   = local_state_t'(u_dut.u_VALVREF_local.current_state);
    assign dut_partner_state = partner_state_t'(u_dut.u_VALVREF_partner.current_state);

    function string get_short_msg_name(msg_no_e msg);
        case (msg)
            MBTRAIN_VALVREF_start_req  : return "START_REQ";
            MBTRAIN_VALVREF_start_resp : return "START_RESP";
            MBTRAIN_VALVREF_end_req    : return "END_REQ";
            MBTRAIN_VALVREF_end_resp   : return "END_RESP";
    ...
    task automatic run_scenario(
            ...
            input logic expect_datavref_dut,
        ...
        // Enable FSMs
        dut_local_valvref_en = 1;
        ptn_local_valvref_en = 1;
        dut_partner_valvref_en = 1;
        ptn_partner_valvref_en = 1;
        ...
        if (expect_datavref_dut) begin
            ...
            begin
                ...
                if (holes_en) begin
                    automatic logic [6:0] hole_pos = d_start + (d_end - d_start)/4;
                    expected_best = (hole_pos + 1 + d_end) / 2;
                end else begin
                    expected_best = (d_start + d_end) / 2;
                end
                if (u_dut.phy_rx_valvref_ctrl !== expected_best) begin
                    $display("# ERROR: Calibrated midpoint mismatch! Obtained=%0d, Expected=%0d", u_dut.phy_rx_valvref_ctrl, expected_best);
                    fail_count++; $stop;
                end
            end
        end
        ...
        dut_local_valvref_en = 0;
        ptn_local_valvref_en = 0;
        dut_partner_valvref_en = 0;
        ptn_partner_valvref_en = 0;
    ...
    initial begin
        ...
        for (int i = 1; i <= 100; i = i + 1) begin
            ...
            begin
                automatic logic [6:0] expected_best;
                if (holes_rnd) begin
                    automatic logic [6:0] hole_pos = start_rnd + (end_rnd - start_rnd)/4;
                    expected_best = (hole_pos + 1 + end_rnd) / 2;
                end else begin
                    expected_best = (start_rnd + end_rnd) / 2;
                end
                if (u_dut.phy_rx_valvref_ctrl !== expected_best) begin
                    $display("# ERROR: Randomized test %0d midpoint mismatch! Obtained=%0d, Expected=%0d", i, u_dut.phy_rx_valvref_ctrl, expected_best);
                    $stop;
                end
            end
        ...
        $display("\n=========================================================");
        $display(" VALVREF WRAPPER TB COMPLETE");
