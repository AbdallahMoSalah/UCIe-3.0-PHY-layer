`timescale 1ps/1ps
module wrapper_DATATRAINVREF_tb;
    ...
    parameter MAX_DATA_VREF_CODE   = 7'D25  ; // Reduced for fast simulation
    ...
    localparam integer TB_SWEEP_CYCLES    = (MAX_DATA_VREF_CODE - MIN_DATA_VREF_CODE + 1) * TB_CYCLES_PER_CODE;
    ...
    localparam int TB_VW = $clog2(MAX_DATA_VREF_CODE + 1);

    wire [TB_VW-1:0] dut_swept_code_sliced;
    wire [TB_VW-1:0] dut_best_code_sliced [0:15];
    assign dut_swept_code_sliced = dut_if.swept_code[TB_VW-1:0];
    for (genvar i = 0; i < 16; i++) begin
        assign dut_best_code_sliced[i] = dut_if.best_code[i][TB_VW-1:0];
    end

    wire [TB_VW-1:0] ptn_swept_code_sliced;
    wire [TB_VW-1:0] ptn_best_code_sliced [0:15];
    assign ptn_swept_code_sliced = ptn_if.swept_code[TB_VW-1:0];
    for (genvar i = 0; i < 16; i++) begin
        assign ptn_best_code_sliced[i] = ptn_if.best_code[i][TB_VW-1:0];
    end
    ...
    wrapper_DATATRAINVREF #(
        ...
        .local_datatrainvref_done        (dut_local_datatrainvref_done),
        ...
        .partner_datatrainvref_done      (dut_partner_datatrainvref_done),
    ...
    wrapper_DATATRAINVREF #(
        ...
        .local_datatrainvref_done        (ptn_local_datatrainvref_done),
        ...
        .partner_datatrainvref_done      (ptn_partner_datatrainvref_done),
    ...
    typedef enum reg [3:0] {
        TB_DTVREF_LOCAL_IDLE           = 4'd0,
        TB_DTVREF_LOCAL_SEND_START_REQ = 4'd1,
        TB_DTVREF_LOCAL_WAIT_START_RESP= 4'd2,
        TB_DTVREF_LOCAL_SWEEP          = 4'd3,
        TB_DTVREF_LOCAL_APPLY_BEST     = 4'd4,
        TB_DTVREF_LOCAL_SEND_END_REQ   = 4'd5,
        TB_DTVREF_LOCAL_WAIT_END_RESP  = 4'd6,
        TB_DTVREF_LOCAL_TO_RXDESKEW    = 4'd7,
        TB_DTVREF_LOCAL_TO_TRAINERROR  = 4'd8
    } tb_local_state_t;
    ...
    initial begin
        dut_if.state_n[0]            = ltsm_state_n_pkg::LOG_MBTRAIN_DATATRAINVREF;
        ...
        ptn_if.state_n[0]            = ltsm_state_n_pkg::LOG_MBTRAIN_DATATRAINVREF;
    ...
    task automatic tb_run_scenario(
        ...
        dut_local_datatrainvref_en = 1;
        ptn_partner_datatrainvref_en = 1;
        ...
        fork
            begin
                wait (u_dut.local_datatrainvref_done || u_dut.local_trainerror_req);
                #(LCLK_PERIOD * 10);
            end
            begin
        ...
        if (expect_success) begin
            if (!u_dut.local_datatrainvref_done || u_dut.local_trainerror_req) begin
            ...
            for (int l = 0; l < 16; l = l + 1) begin
                ...
                if (u_dut.phy_rx_datavref_ctrl[l] !== expected_best) begin
                    $display("# ERROR: Lane %0d mismatch! Got=%0d, Exp=%0d", l, u_dut.phy_rx_datavref_ctrl[l], expected_best);
                    tb_fail_count++; $stop;
                end
            end
        ...
        dut_local_datatrainvref_en = 0;
        ptn_partner_datatrainvref_en = 0;
    ...
    initial begin
        ...
        for (int l = 0; l < 16; l = l + 1) begin
            standard_start[l] = 12; standard_end[l] = 22;
        end

        $display("# Running wrapper_DATATRAINVREF_tb");
        ...
        for (int i = 1; i <= 20; i = i + 1) begin
            ...
            dut_local_datatrainvref_en = 1; ptn_partner_datatrainvref_en = 1;
            wait (u_dut.local_datatrainvref_done || u_dut.local_trainerror_req);
            #10000;
            dut_local_datatrainvref_en = 0; ptn_partner_datatrainvref_en = 0;
            #10000;
        end
