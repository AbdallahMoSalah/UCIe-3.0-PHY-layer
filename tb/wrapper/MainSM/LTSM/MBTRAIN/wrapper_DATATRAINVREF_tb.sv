...
module wrapper_DATATRAINVREF_tb;
    ...
    always @(posedge lclk or negedge rst_n) begin
        if (!rst_n) begin
            tb_prev_dut_local_state   <= DATATRAINVREF_LCL_IDLE;
            tb_prev_dut_partner_state <= DATATRAINVREF_PTR_IDLE;
        end else begin
            if (!tb_in_randomized_scenarios || ENABLE_RAND_LOG) begin
                if (tb_dut_local_state !== tb_prev_dut_local_state) begin
                    $display("# [%0d ps] Die A LOCAL   State -> %s", $realtime(), tb_dut_local_state.name());
                    tb_prev_dut_local_state <= tb_dut_local_state;
                end
                if (tb_dut_partner_state !== tb_prev_dut_partner_state) begin
                    $display("# [%0d ps] Die A PARTNER State -> %s", $realtime(), tb_dut_partner_state.name());
                    tb_prev_dut_partner_state <= tb_dut_partner_state;
                end
            end else begin
                if (tb_dut_local_state !== tb_prev_dut_local_state) tb_prev_dut_local_state <= tb_dut_local_state;
                if (tb_dut_partner_state !== tb_prev_dut_partner_state) tb_prev_dut_partner_state <= tb_dut_partner_state;
            end
        end
    end

    initial begin
        dut_if.state_n_0             = ltsm_state_n_pkg::LOG_MBTRAIN_DATATRAINVREF;
        ...
        ptn_if.state_n_0             = ltsm_state_n_pkg::LOG_MBTRAIN_DATATRAINVREF;
    ...
    task automatic tb_run_scenario(
        ...
        ptn_local_datatrainvref_en = 1;
        dut_partner_datatrainvref_en = 1;
        ...
        fork
            begin
                wait (dut_datatrainvref_done || dut_trainerror_req);
                #(LCLK_PERIOD * 10);
            end
            begin
        ...
        if (expect_success) begin
            if (!dut_datatrainvref_done || dut_trainerror_req) begin
        ...
        ptn_local_datatrainvref_en = 0;
        dut_partner_datatrainvref_en = 0;
