`timescale 1ps/1ps
module wrapper_VALTRAINVREF_tb;
    ...
    typedef enum reg [3:0] {
        VALTRAINVREF_PTR_IDLE            = 4'd0,
        VALTRAINVREF_PTR_WAIT_START_REQ  = 4'd1,
    ...
    function string get_short_msg_name(msg_no_e msg);
        case (msg)
            MBTRAIN_VALTRAINVREF_start_req  : return "START_REQ";
            MBTRAIN_VALTRAINVREF_start_resp : return "START_RESP";
            MBTRAIN_VALTRAINVREF_end_req    : return "END_REQ";
            MBTRAIN_VALTRAINVREF_end_resp   : return "END_RESP";
    ...
    task automatic run_scenario(
        ...
        if (expect_done_dut) begin
            ...
            begin
                automatic logic [VREF_W-1:0] expected_best;
                if (holes_en) begin
                    automatic logic [VREF_W-1:0] hole_pos = d_start + (d_end - d_start)/4;
    ...
    initial begin
        $display("# =========================================================");
        $display("# Running wrapper_VALTRAINVREF_tb                          ");
        $display("# =========================================================");
        ...
        for (int i = 1; i <= 100; i = i + 1) begin
            ...
            begin
                automatic logic [VREF_W-1:0] expected_best;
                if (holes_rnd) begin
                    automatic logic [VREF_W-1:0] hole_pos = start_rnd + (end_rnd - start_rnd)/4;
        ...
        $display("\n=========================================================");
        $display(" VALTRAINVREF WRAPPER TB COMPLETE");
