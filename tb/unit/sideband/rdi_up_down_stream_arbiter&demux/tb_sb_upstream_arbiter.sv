...
module tb_sb_upstream_arbiter();
    ...
    task check_result(input logic exp_vld, input logic [127:0] exp_msg, input logic exp_comp_rdy, string test_name);
        if (Adapter_vld === exp_vld && Adapter_msg === exp_msg && completion_rdy === exp_comp_rdy)
    ...
    initial begin
        ...
        // Expected: Remote passes, completion_rdy = 0
        // --------------------------------------------------------
        ...
        // Expected: Local passes, completion_rdy = 1 (أهم حاجة الـ Rdy بواحد)
        // --------------------------------------------------------
        ...
        // Expected: Remote MUST win (Msg = AAAA...), Local MUST stall (completion_rdy = 0)
        // --------------------------------------------------------
