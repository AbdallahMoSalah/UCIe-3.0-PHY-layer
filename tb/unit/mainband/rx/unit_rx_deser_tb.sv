`timescale 1ns/1ps
// =============================================================================
// Testbench : unit_rx_deser_tb_s3
// Purpose   : Verification of Solution 3 (Posedge Align + 16-Cycle Counter)
// =============================================================================
module unit_rx_deser_tb_s3;
    ...
    unit_mb_serializer #(.DATA_WIDTH(DATA_WIDTH)) u_tx_data_ser (
        .mb_clk  (mb_clk),
        .PLL_clk (pll_clk),
        .i_rst_n (rst_n),
        .Ser_en  (tx_ser_en),
    ...
    // RX: Valid Deserializer (s3: posedge align + counter)
    unit_valid_deserializer_s3 #(.DATA_WIDTH(DATA_WIDTH)) u_valid_des (
        .pll_clk     (rx_pll_clk), // Use delayed clock
        .i_rst_n     (rst_n),
        .ser_data_in (valid_serial_out),
        .o_shift_reg (valid_shift_reg),
        .o_state     (valid_des_state),
        .o_count     (valid_des_count)
    );

    // RX: Valid Frame Detector (s3: gated match)
    unit_valid_frame_detector_s3 #(
        .DATA_WIDTH   (DATA_WIDTH),
        .VALID_PATTERN(32'h0F0F0F0F)
    ) u_frame_det (
        .i_shift_reg         (valid_shift_reg),
        .i_state             (valid_des_state),
        .i_count             (valid_des_count),
        .o_valid_frame_pulse (valid_frame_pulse)
    );

    // RX: Data Deserializer
    unit_data_deserializer_s3 #(.DATA_WIDTH(DATA_WIDTH)) u_data_des (
        .mb_clk              (mb_clk),
        .pll_clk             (rx_pll_clk), // Use delayed clock
        .i_rst_n             (rst_n),
    ...
    initial begin
        $dumpfile("rx_deser_tb_s3.vcd");
        $dumpvars(0, unit_rx_deser_tb_s3);
        ...
        $display("");
        $display("==========================================================");
        $display("  RX Deserializer Solution 3 Testbench");
