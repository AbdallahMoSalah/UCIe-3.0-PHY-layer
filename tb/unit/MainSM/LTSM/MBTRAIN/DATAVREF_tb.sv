...
module DATAVREF_tb ();
    ...
    typedef enum reg [3:0] {
        DATAVREF_IDLE          = DATAVREF_inst.DATAVREF_IDLE         , // (S0)
        DATAVREF_START_REQ     = DATAVREF_inst.DATAVREF_START_REQ    , // (S1)
        DATAVREF_START_RESP    = DATAVREF_inst.DATAVREF_START_RESP   , // (S2)
        DATAVREF_SET_VREF_CODE = DATAVREF_inst.DATAVREF_SET_VREF_CODE, // (S3)
        DATAVREF_RX_D2C_PT     = DATAVREF_inst.DATAVREF_RX_D2C_PT    , // (S4)
        DATAVREF_LOG_RESULT    = DATAVREF_inst.DATAVREF_LOG_RESULT   , // (S5)
        DATAVREF_CALC_APPLY    = DATAVREF_inst.DATAVREF_CALC_APPLY   , // (S6)
        DATAVREF_END_REQ       = DATAVREF_inst.DATAVREF_END_REQ      , // (S7)
        DATAVREF_END_RESP      = DATAVREF_inst.DATAVREF_END_RESP     , // (S8)
        TO_SPEEDIDLE           = DATAVREF_inst.TO_SPEEDIDLE          , // (S9)
        TO_TRAINERROR          = DATAVREF_inst.TO_TRAINERROR         , // (S10)
        Continue_Repeating_The_Last_3_States = 'hF
    } fsm_state_t;
    fsm_state_t current_state, monitor_current_state;

    assign current_state  = fsm_state_t'(DATAVREF_inst.current_state);
