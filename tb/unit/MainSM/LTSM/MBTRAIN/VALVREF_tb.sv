...
module VALVREF_tb ();
    ...
    // |  -------------------------                (assume_errors)               ---------------------------  |
    //  \______________________/‾‾‾‾‾\________________________________________/‾‾‾‾‾\________________________/
    logic [2:0] clk_sampling_easier_form; // Choose the part of the Eye diagram that the clock samples Valid lane signal correctly. (3'b010: Eye Center;  3'b100: Left edge;  3'b001: Right edge;  3'b101: Right edge and left edge)
    
    // Some signals to store the and move the signals for the next always block.
    reg [2:0]                 current_task_clk_sampling;
    ...
    task assume_errors (
        ...
        input [2:0]  task_clk_sampling = 3'b010, // Choose the part of the Eye diagram that the clock samples Valid lane signal correctly. (3'b010: Eye Center;  3'b100: Left edge;  3'b001: Right edge;  3'b101: Right edge and left edge)
        input [VREF_CODE_WIDTH-1:0] task_vref_code_min  = 'd50 ,
        input [VREF_CODE_WIDTH-1:0] task_vref_code_max  = 'd100,
        ...
        current_task_clk_sampling = task_clk_sampling ;
        current_task_vref_min     = task_vref_code_min;
        current_task_vref_max     = task_vref_code_max;

        clk_sampling_easier_form = task_clk_sampling;
    ...
    always @(*) begin
        if(intf.phy_rx_valvref_ctrl >= current_task_vref_min && intf.phy_rx_valvref_ctrl <= current_task_vref_max) begin
            ...
            if ((intf.phy_rx_valvref_ctrl == current_task_vref_min + (current_task_vref_max - current_task_vref_min)/4 ) &&
                 assume_holes_after_quarter_eye_start == 1 &&
                 current_task_vref_min != current_task_vref_max)begin
                intf.tb_val_err = 1'b1;  // A deliberate mistake in the middle!
            end                                                                                
            // =============================================================================================== //

            else if((intf.d2c_clk_sampling == 2'd0 && current_task_clk_sampling[1] == 1'b1) ||     // (Eye Center)
                    (intf.d2c_clk_sampling == 2'd1 && current_task_clk_sampling[2] == 1'b1) ||     // (Left  edge)
                    (intf.d2c_clk_sampling == 2'd2 && current_task_clk_sampling[0] == 1'b1)) begin // (Right edge)
                
                intf.tb_val_err = 1'b0; // The right point that is inside the Eye Diagram.
            end
            else begin
                intf.tb_val_err = 1'b1; // Error in Eye Clock test.
            end
        end
        else begin
            intf.tb_val_err = 1'b1; // Vref value is outside the right bound
        end
    end
        ...
        fork : test_execution
            begin
                ...
                if(intf.trainerror_req != 1'b1) begin
                    ...
                    hole_pos             = (assume_holes_after_quarter_eye_start && (current_task_vref_min != current_task_vref_max))? current_task_vref_min + (current_task_vref_max - current_task_vref_min)/4 : (current_task_vref_min-1);
                    expected_best_center = ({1'b0, hole_pos + 1} + {1'b0, current_task_vref_max}) / 2;

                    if((intf.valvref_fail_flag == 1'b0 && clk_sampling_easier_form == 3'b000) ||
                       (intf.valvref_fail_flag == 1'b1 && (|clk_sampling_easier_form))        ||
                       (intf.valvref_fail_flag == 1'b0 && (|clk_sampling_easier_form) && intf.phy_rx_valvref_ctrl != expected_best_center ) ) begin
                        
                        repeat(5) $display("\t\t ************************** ERROR **************************");
                        $display("error valvref_fail_flag = %0d, clk_sampling = %03b, intf.phy_rx_valvref_ctrl = %0d, Expected Center = %0d, is_there_holes = %0b", 
                        intf.valvref_fail_flag, 
                        clk_sampling_easier_form, 
    ...
    initial begin
        ...
        $display("\n=========>  Test Scenario (%0d): Happy Scenario. <=========", test_scenario_no++);
            assume_errors (
                ...
                .task_clk_sampling (3'b100 >> i), // Choose the part of the Eye diagram that the clock samples Valid lane signal correctly. (3'b010: Eye Center;  3'b100: Left edge;  3'b001: Right edge;  3'b101: Right edge and left edge)
                .task_vref_code_min(),
                .task_vref_code_max('d100   ),
        ...
        assume_errors (
            ...
            // .task_val_err      (1'b0    ), // The valid lane error to be assumed for the test scenario.
            // .task_clk_err      (1'b0    )  // The clock lane error to be assumed for the test scenario.
            .task_clk_sampling (3'b111 ), // Choose the part of the Eye diagram that the clock samples Valid lane signal correctly. (3'b010: Eye Center;  3'b100: Left edge;  3'b001: Right edge;  3'b101: Right edge and left edge)
        ...
        /////////////////////////////////////////////////////////////////////////////////////////////////
        // The test scenario 18:30 or (18:1000 but it will take 8:15 min)): Check Holes Scenario.      //  
        /////////////////////////////////////////////////////////////////////////////////////////////////
        // for(int i = 18; i <= 1000; i++) begin
        for(int i = 18; i <= 30; i++) begin
        $display("\n=========>  Test Scenario (%0d): Happy Scenario. <=========", test_scenario_no++);
            temporary_var = 0;
            ...
            assume_errors (
                ...
                // .task_val_err      (1'b0    ), // The valid lane error to be assumed for the test scenario.
                // .task_clk_err      (1'b0    )  // The clock lane error to be assumed for the test scenario.
                .task_clk_sampling (i[2:0]                         ), // Choose the part of the Eye diagram that the clock samples Valid lane signal correctly. (3'b010: Eye Center;  3'b100: Left edge;  3'b001: Right edge;  3'b101: Right edge and left edge)
