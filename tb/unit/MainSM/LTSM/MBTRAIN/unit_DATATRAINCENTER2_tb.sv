`timescale 1ps/1ps
// =============================================================================
// Testbench : unit_DATATRAINCENTER2_tb
// Purpose   : Self-checking testbench for unit_DATATRAINCENTER2 FSM.
//
//             Mirrors the structure and coverage of unit_DATATRAINCENTER1_tb,
//             adapted for the DTC2 state names and LINKSPEED next state.
//
// Scenarios:
//   ✓ Happy path: full per-lane phase sweep → midpoints applied & verified
//   ✓ SB connection interruption (8ms hardware timeout → TO_TRAINERROR)
//   ✓ Partner TRAINERROR SB message → TO_TRAINERROR
//   ✓ Wrong SB message → FSM stalls → 8ms timeout
//   ✓ Randomized per-lane hole-in-eye + random lane masks
// =============================================================================
module unit_DATATRAINCENTER2_tb ();
    import UCIe_pkg::*;

    // ─── Parameters ──────────────────────────────────────────────────────────
    // Decrease MAX_PHASE_CODE (e.g. to 50) to shorten simulation time.
    parameter LCLK_PERIOD          = 1*1000 ; // 1 ns at 1 GHz (ps timescale)
    parameter ANALOG_SETTLE_CYCLES = 10     ;
    parameter MIN_PHASE_CODE       = 7'd0   ;
    parameter MAX_PHASE_CODE       = 7'd50  ; // Reduced for simulation speed (DUT default is 127)
    ...
    typedef enum reg [3:0] {
        DTC2_IDLE        = unit_DATATRAINCENTER2_inst.DTC2_IDLE       ,
        DTC2_START_REQ   = unit_DATATRAINCENTER2_inst.DTC2_START_REQ  ,
        DTC2_START_RESP  = unit_DATATRAINCENTER2_inst.DTC2_START_RESP ,
        DTC2_SET_PHASE   = unit_DATATRAINCENTER2_inst.DTC2_SET_PHASE  ,
        DTC2_TX_D2C_PT   = unit_DATATRAINCENTER2_inst.DTC2_TX_D2C_PT  ,
        DTC2_LOG_RESULT  = unit_DATATRAINCENTER2_inst.DTC2_LOG_RESULT ,
        DTC2_CALC_APPLY  = unit_DATATRAINCENTER2_inst.DTC2_CALC_APPLY ,
        DTC2_END_REQ     = unit_DATATRAINCENTER2_inst.DTC2_END_REQ    ,
        DTC2_END_RESP    = unit_DATATRAINCENTER2_inst.DTC2_END_RESP   ,
        TO_LINKSPEED     = unit_DATATRAINCENTER2_inst.TO_LINKSPEED    ,
        TO_TRAINERROR    = unit_DATATRAINCENTER2_inst.TO_TRAINERROR   ,
        Continue_Repeating_The_Last_3_States = 'hF
    } fsm_state_t;

    fsm_state_t current_state, monitor_current_state;
    assign current_state = fsm_state_t'(unit_DATATRAINCENTER2_inst.current_state);
    ...
    task start_test(
        ...
        fork : test_execution
            ...
            begin
                intf.datatraincenter2_en = 1'b1;
                wait(intf.datatraincenter2_done || intf.trainerror_req); #1step;

                intf.datatraincenter2_en = 1'b0;
                ...
                if (!intf.trainerror_req) begin
                    ...
                    wait(current_state == DTC2_IDLE); #1step;
                end else begin
            ...
            begin : check_fsm_transitions
                wait(current_state == DTC2_IDLE);
                entered_states[0] = 1;
                wait(current_state == DTC2_START_REQ);
                entered_states[1] = 1;
                wait(current_state == DTC2_START_RESP);
                entered_states[2] = 1;
                repeat(MAX_PHASE_CODE - MIN_PHASE_CODE + 1) begin
                    wait(current_state == DTC2_SET_PHASE);
                    entered_states[3] = 1;
                    wait(current_state == DTC2_TX_D2C_PT);
                    entered_states[4] = 1;
                    wait(current_state == DTC2_LOG_RESULT);
                    entered_states[5] = 1;
                end
                wait(current_state == DTC2_CALC_APPLY);
                entered_states[6] = 1;
                wait(current_state == DTC2_END_REQ);
                entered_states[7] = 1;
                wait(current_state == DTC2_END_RESP);
                entered_states[8] = 1;
                wait(current_state == TO_LINKSPEED);
                entered_states[9] = 1;
                wait(current_state == DTC2_IDLE);
                entered_states[10] = 1;
            end
