...
//   ✓ dtc2_fail_flag set when ALL lanes fail the entire sweep (no pass found)
...
module unit_DATATRAINCENTER2_tb ();
    ...
    task start_test(
        ...
        fork : TEST_EXEC
            begin
                ...
                // NOTE: datatraincenter2_fail_flag is currently not wired out of
                // the DUT (signal commented-out in unit_DATATRAINCENTER2.sv).
                // When the flag is re-enabled, restore the assertion below:
                //   if (!expect_trainerror && intf.datatraincenter2_fail_flag != expect_fail_flag)
                //       $stop;

                wait(current_state == DTC2_IDLE || current_state == TO_TRAINERROR); #1step;
    ...
    initial begin
        ...
        // ─── Scenario 2: All lanes fail entire sweep → dtc2_fail_flag = 1 ─
        ...
        // ─── Scenarios 5-204: 200 Randomized per-lane eye positions ────────────
        for (int s = 5; s <= 204; s++) begin
