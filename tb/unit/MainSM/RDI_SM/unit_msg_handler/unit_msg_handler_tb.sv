...
module unit_msg_handler_tb();
  ...
  logic Active_req_s;
  logic valid_r;
msg_no_e Link_Mgmt_Msg_Received;
  ...
  unit_msg_handler uut (
    ...
    .valid_r(valid_r),
    .Link_Mgmt_Msg_Received(Link_Mgmt_Msg_Received),
  ...
  initial begin
    ...
    Active_req_s = 0;
    valid_r = 0;
    Link_Mgmt_Msg_Received = NOTHING;
    ...
    @(posedge lclk);
    valid_r = 1;
    Link_Mgmt_Msg_Received = RDI_L1_RSP;
    @(posedge lclk);
    valid_r = 0;
    Link_Mgmt_Msg_Received = NOTHING;
    repeat(3) @(posedge lclk);
    ...
    @(posedge lclk);
    valid_r = 1;
    Link_Mgmt_Msg_Received = RDI_ACTIVE_REQ;
    @(posedge lclk);
    valid_r = 0;
    Link_Mgmt_Msg_Received = NOTHING;
    ...
    @(posedge lclk);
    valid_r = 1;
    Link_Mgmt_Msg_Received = RDI_ACTIVE_RSP;
    @(posedge lclk);
    valid_r = 0;
    Link_Mgmt_Msg_Received = NOTHING;
    repeat(5) @(posedge lclk);
    ...
    Message_send = RDI_L2_REQ; // Change to MSG2 immediately
    @(posedge lclk); 
    // SM goes to IDLE
    @(posedge lclk); 
  ...
  initial begin
    $monitor("Time=%0t | cs=%0s | valid_s=%b | Send_Msg=%0s | Receive_Msg=%0s | req_r=%b | resp_r=%b", 
             $time, uut.cs.name(), valid_s, Link_Mgmt_Msg_Send.name(), Message_receive.name(), Active_req_r, Active_resp_r);
  end
