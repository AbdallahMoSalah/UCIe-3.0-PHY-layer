//-----------------------------------------------------------------------------
// Module      : unit_L2_state_tb
// Description : Testbench for unit_L2_state module.
//               Covers 8+ transitions from the L2 state machine diagram.
//-----------------------------------------------------------------------------
`timescale 1ns/1ps
import RDI_SM_pkg::*;
import UCIe_pkg::*;
...
module unit_L2_state_tb();
    ...
    initial begin
        ...
        $display("=================================================");
        $display("       Starting unit_L2_state Simulation        ");
        $display("=================================================");

        // ---------------------------------------------------------------------
        // Scenario 1: Reset/Enable Logic
        // ---------------------------------------------------------------------
        ...
        lp_state_req = Active;
        #10;
        check_condition((active_handshake_strt == 1'b1), "active_handshake_strt not pulsed on Active request");
        check_condition((state_req == ACTIVE),           "state_req not ACTIVE on Active request");
        check_condition((uut.cs == 4'd8),                "DUT did not reach training state");
        
        #10;
        lp_state_req = Nop;
        massage_receive = RDI_ACTIVE_REQ; // Peer responds with ACTIVE_REQ as per diagram
        #10;
        check_condition((uut.cs == 4'd9),                "DUT did not reach reset state after Peer REQ");
        check_condition((next_state == Active),          "next_state not Active in reset state");
        massage_receive = NOP;
        ...
        #10;
        check_condition((state_req == ACTIVE),           "state_req not ACTIVE on Peer Active request");
        check_condition((uut.cs == 4'd9),                "DUT did not reach reset state (Peer initiated)");
        massage_receive = NOP;
        ...
        #10;
        check_condition((uut.cs == 4'd11),             "DUT did not reach linkerror state");
        ...
        #10;
        #10
        check_condition((uut.cs == 4'd10),               "DUT did not reach linkreset state");
        ...
        #10;
        check_condition((uut.cs == 4'd12),             "DUT did not reach disabled state");
