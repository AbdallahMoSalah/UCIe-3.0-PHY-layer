`timescale 1ns/1ps
// =============================================================================
// Testbench : unit_mb_die2die_ltsm_tb
// DUT       : two unit_mb_die instances integrated with mainband_ltsm_interface
//             wired back-to-back (die 0 <-> die 1)
// =============================================================================

module unit_mb_die2die_ltsm_tb;
    ...
    // Non-interface controls
    logic                  i_pll_en;
    logic [1:0]            i_pll_speed_sel;
    logic                  lclk_g;
    logic                  i_clk_embedded_en;
    logic [15:0]           i_pcmp_thr_per_lane, i_pcmp_thr_aggregate;
    logic [15:0]           i_vcmp_thr;

    // Interface shared inputs
    logic                  tb_active = 0;
    logic                  tb_mb_tx_pattern_en = 0;
    logic [2:0]            tb_mb_tx_pattern_setup = 3'b000;
    logic [1:0]            tb_mb_tx_data_pattern_sel = 2'b00;
    logic                  tb_mb_tx_val_pattern_sel = 0;
    logic                  tb_mb_rx_compare_en = 0;
    logic [1:0]            tb_mb_rx_compare_setup = 2'b00;
    logic                  tb_clear_error_req = 0;
    logic                  tb_mb_lane_reversal_req = 0;
    logic                  tb_mb_tx_lfsr_rst = 0;
    logic                  tb_mb_rx_lfsr_rst = 0;
    logic                  tb_mb_rx_vcomp_mode = 0;
    logic                  tb_mb_rx_data_en = 0;
    logic                  tb_mb_rx_valid_en = 0;
    logic [NUM_LANES-1:0]  tb_reg_lane_mask0 = '0;
    logic [NUM_LANES-1:0]  tb_reg_lane_mask1 = '0;
    ...
    // ============================================================== LTSM 0
    logic                    i_mapper_en0;
    logic [2:0]              i_width_deg_tx0;
    logic [2:0]              i_width_deg_rx0;
    logic [2:0]              i_lfsr_state0;
    logic                    i_reversal_en0;
    logic                    i_valid_pattern_en0;
    logic                    i_clk_pattern_en0;
    logic [2:0]              i_state0;
    logic                    demapper_en0;
    logic                    i_pcmp_enable0;
    logic                    i_pcmp_mode0;
    logic [NUM_LANES-1:0]    i_pcmp_lane_mask0;
    logic [15:0]             i_pcmp_iter_count0;
    logic                    i_pcmp_pattern_mode0;
    logic                    i_pcmp_clear0;
    logic                    i_vcmp_enable0;
    logic                    i_vcmp_mode0;
    logic                    i_vcmp_clear0;
    logic                    i_clk_detector_en0;
    logic [NUM_LANES-1:0]    i_rx_data_deser_en0;
    logic                    i_rx_valid_deser_en0;

    logic                    o_lfsr_tx_done0;
    logic                    o_valid_done0;
    logic                    o_clk_done0;
    logic                    o_pcmp_done0;
    logic [NUM_LANES-1:0]    o_pcmp_per_lane_pass0;
    logic                    o_vcmp_done0;
    logic                    o_vcmp_pass0;
    logic                    o_clk_p_pass0;
    logic                    o_clk_n_pass0;
    ...
    logic [NUM_LANES-1:0]    mb_rx_perlane_pass0;
    logic                    mb_tx_pattern_count_done0;
    logic                    repairclk_rtrk_pass0;
    logic                    repairclk_rckn_pass0;
    logic                    repairclk_rckp_pass0;
    logic                    repairval_RVLD_L_pass0;
    logic                    mb_rx_compare_done0;

    mainband_ltsm_interface #(
        .NUM_LANES(NUM_LANES)
    ) ltsm_if0 (
        .i_mapper_en(i_mapper_en0),
        .i_width_deg_tx(i_width_deg_tx0),
        .i_width_deg_rx(i_width_deg_rx0),
        .i_lfsr_state(i_lfsr_state0),
        .i_reversal_en(i_reversal_en0),
        .i_valid_pattern_en(i_valid_pattern_en0),
        .i_clk_pattern_en(i_clk_pattern_en0),
        .i_state(i_state0),
        .demapper_en(demapper_en0),
        .i_pcmp_enable(i_pcmp_enable0),
        .i_pcmp_mode(i_pcmp_mode0),
        .i_pcmp_lane_mask(i_pcmp_lane_mask0),
        .i_pcmp_iter_count(i_pcmp_iter_count0),
        .i_pcmp_pattern_mode(i_pcmp_pattern_mode0),
        .i_pcmp_clear(i_pcmp_clear0),
        .i_vcmp_enable(i_vcmp_enable0),
        .i_vcmp_mode(i_vcmp_mode0),
        .i_vcmp_clear(i_vcmp_clear0),
        .i_clk_detector_en(i_clk_detector_en0),
        .i_rx_data_deser_en(i_rx_data_deser_en0),
        .i_rx_valid_deser_en(i_rx_valid_deser_en0),

        .o_lfsr_tx_done(o_lfsr_tx_done0),
        .o_valid_done(o_valid_done0),
        .o_clk_done(o_clk_done0),
        .o_pcmp_done(o_pcmp_done0),
        .o_pcmp_per_lane_pass(o_pcmp_per_lane_pass0),
        .o_vcmp_done(o_vcmp_done0),
        .o_vcmp_pass(o_vcmp_pass0),
        .o_valid_frame_error(o_valid_frame_error0),
        .o_clk_p_pass(o_clk_p_pass0),
        .o_clk_n_pass(o_clk_n_pass0),
        .o_track_pass(o_track_pass0),
        .reg_lane_mask(tb_reg_lane_mask0),

        .mb_tx_pattern_en(tb_mb_tx_pattern_en),
        .mb_tx_pattern_setup(tb_mb_tx_pattern_setup),
        .mb_tx_data_pattern_sel(tb_mb_tx_data_pattern_sel),
        .mb_tx_val_pattern_sel(tb_mb_tx_val_pattern_sel),
        .mb_rx_compare_en(tb_mb_rx_compare_en),
        .mb_rx_compare_setup(tb_mb_rx_compare_setup),
        .clear_error_req(tb_clear_error_req),
        .mb_rx_data_lane_map(die0_width_deg_rx),
        .mb_tx_data_lane_map(die0_width_deg_tx),

        .mb_rx_perlane_pass(mb_rx_perlane_pass0),
        .mb_tx_pattern_count_done(mb_tx_pattern_count_done0),

        .mb_lane_reversal_req(tb_mb_lane_reversal_req),
        .active(tb_active),
        .mb_tx_lfsr_rst(tb_mb_tx_lfsr_rst),
        .mb_rx_lfsr_rst(tb_mb_rx_lfsr_rst),
        .mb_rx_vcomp_mode(tb_mb_rx_vcomp_mode),
        .mb_rx_data_en(tb_mb_rx_data_en),
        .mb_rx_valid_en(tb_mb_rx_valid_en),
        .repairclk_rtrk_pass(repairclk_rtrk_pass0),
        .repairclk_rckn_pass(repairclk_rckn_pass0),
        .repairclk_rckp_pass(repairclk_rckp_pass0),
        .repairval_RVLD_L_pass(repairval_RVLD_L_pass0),
        .mb_rx_compare_done(mb_rx_compare_done0)
    );
    ...
    unit_mb_die #(
        ...
        .i_mapper_en(i_mapper_en0), .i_width_deg_tx(i_width_deg_tx0), .i_width_deg_rx(i_width_deg_rx0), .i_lfsr_state(i_lfsr_state0),
        .i_reversal_en(i_reversal_en0), .i_valid_pattern_en(i_valid_pattern_en0),
        .i_pll_en(i_pll_en), .i_pll_speed_sel(i_pll_speed_sel), .lclk_g(lclk_g),
        .i_clk_pattern_en(i_clk_pattern_en0), .i_clk_embedded_en(i_clk_embedded_en),
        .i_state(i_state0), .demapper_en(demapper_en0),
        .i_pcmp_enable(i_pcmp_enable0), .i_pcmp_mode(i_pcmp_mode0), .i_pcmp_lane_mask(i_pcmp_lane_mask0),
        .i_pcmp_thr_per_lane(i_pcmp_thr_per_lane), .i_pcmp_thr_aggregate(i_pcmp_thr_aggregate),
        .i_pcmp_iter_count(i_pcmp_iter_count0), .i_pcmp_pattern_mode(i_pcmp_pattern_mode0),
        .i_pcmp_clear(i_pcmp_clear0),
        .i_vcmp_enable(i_vcmp_enable0), .i_vcmp_mode(i_vcmp_mode0), .i_vcmp_thr(i_vcmp_thr),
        .i_vcmp_clear(i_vcmp_clear0), .i_clk_detector_en(i_clk_detector_en0), 
        .i_rx_data_deser_en(i_rx_data_deser_en0), .i_rx_valid_deser_en(i_rx_valid_deser_en0),
        // RX in <- die1 TX
        ...
        .o_pcmp_agg_err_cnt(), .o_pcmp_agg_error(),
    ...
    // ============================================================== LTSM 1
    logic                    i_mapper_en1;
    logic [2:0]              i_width_deg_tx1;
    logic [2:0]              i_width_deg_rx1;
    logic [2:0]              i_lfsr_state1;
    logic                    i_reversal_en1;
    logic                    i_valid_pattern_en1;
    logic                    i_clk_pattern_en1;
    logic [2:0]              i_state1;
    logic                    demapper_en1;
    logic                    i_pcmp_enable1;
    logic                    i_pcmp_mode1;
    logic [NUM_LANES-1:0]    i_pcmp_lane_mask1;
    logic [15:0]             i_pcmp_iter_count1;
    logic                    i_pcmp_pattern_mode1;
    logic                    i_pcmp_clear1;
    logic                    i_vcmp_enable1;
    logic                    i_vcmp_mode1;
    logic                    i_vcmp_clear1;
    logic                    i_clk_detector_en1;
    logic [NUM_LANES-1:0]    i_rx_data_deser_en1;
    logic                    i_rx_valid_deser_en1;

    logic                    o_lfsr_tx_done1;
    logic                    o_valid_done1;
    logic                    o_clk_done1;
    logic                    o_pcmp_done1;
    logic [NUM_LANES-1:0]    o_pcmp_per_lane_pass1;
    logic                    o_vcmp_done1;
    logic                    o_vcmp_pass1;
    logic                    o_clk_p_pass1;
    logic                    o_clk_n_pass1;
    ...
    logic [NUM_LANES-1:0]    mb_rx_perlane_pass1;
    logic                    mb_tx_pattern_count_done1;
    logic                    repairclk_rtrk_pass1;
    logic                    repairclk_rckn_pass1;
    logic                    repairclk_rckp_pass1;
    logic                    repairval_RVLD_L_pass1;
    logic                    mb_rx_compare_done1;

    mainband_ltsm_interface #(
        .NUM_LANES(NUM_LANES)
    ) ltsm_if1 (
        .i_mapper_en(i_mapper_en1),
        .i_width_deg_tx(i_width_deg_tx1),
        .i_width_deg_rx(i_width_deg_rx1),
        .i_lfsr_state(i_lfsr_state1),
        .i_reversal_en(i_reversal_en1),
        .i_valid_pattern_en(i_valid_pattern_en1),
        .i_clk_pattern_en(i_clk_pattern_en1),
        .i_state(i_state1),
        .demapper_en(demapper_en1),
        .i_pcmp_enable(i_pcmp_enable1),
        .i_pcmp_mode(i_pcmp_mode1),
        .i_pcmp_lane_mask(i_pcmp_lane_mask1),
        .i_pcmp_iter_count(i_pcmp_iter_count1),
        .i_pcmp_pattern_mode(i_pcmp_pattern_mode1),
        .i_pcmp_clear(i_pcmp_clear1),
        .i_vcmp_enable(i_vcmp_enable1),
        .i_vcmp_mode(i_vcmp_mode1),
        .i_vcmp_clear(i_vcmp_clear1),
        .i_clk_detector_en(i_clk_detector_en1),
        .i_rx_data_deser_en(i_rx_data_deser_en1),
        .i_rx_valid_deser_en(i_rx_valid_deser_en1),

        .o_lfsr_tx_done(o_lfsr_tx_done1),
        .o_valid_done(o_valid_done1),
        .o_clk_done(o_clk_done1),
        .o_pcmp_done(o_pcmp_done1),
        .o_pcmp_per_lane_pass(o_pcmp_per_lane_pass1),
        .o_vcmp_done(o_vcmp_done1),
        .o_vcmp_pass(o_vcmp_pass1),
        .o_valid_frame_error(o_valid_frame_error1),
        .o_clk_p_pass(o_clk_p_pass1),
        .o_clk_n_pass(o_clk_n_pass1),
        .o_track_pass(o_track_pass1),
        .reg_lane_mask(tb_reg_lane_mask1),
        ...
        .mb_rx_compare_setup(tb_mb_rx_compare_setup),
        .clear_error_req(tb_clear_error_req),
        .mb_rx_data_lane_map(die1_width_deg_rx),
        .mb_tx_data_lane_map(die1_width_deg_tx),

        .mb_rx_perlane_pass(mb_rx_perlane_pass1),
        .mb_tx_pattern_count_done(mb_tx_pattern_count_done1),
        ...
        .mb_rx_data_en(tb_mb_rx_data_en),
        .mb_rx_valid_en(tb_mb_rx_valid_en),
        .repairclk_rtrk_pass(repairclk_rtrk_pass1),
        .repairclk_rckn_pass(repairclk_rckn_pass1),
        .repairclk_rckp_pass(repairclk_rckp_pass1),
        .repairval_RVLD_L_pass(repairval_RVLD_L_pass1),
        .mb_rx_compare_done(mb_rx_compare_done1)
    );
    ...
    unit_mb_die #(
        ...
        .i_mapper_en(i_mapper_en1), .i_width_deg_tx(i_width_deg_tx1), .i_width_deg_rx(i_width_deg_rx1), .i_lfsr_state(i_lfsr_state1),
        .i_reversal_en(i_reversal_en1), .i_valid_pattern_en(i_valid_pattern_en1),
        .i_pll_en(i_pll_en), .i_pll_speed_sel(i_pll_speed_sel), .lclk_g(lclk_g),
        .i_clk_pattern_en(i_clk_pattern_en1), .i_clk_embedded_en(i_clk_embedded_en),
        .i_state(i_state1), .demapper_en(demapper_en1),
        .i_pcmp_enable(i_pcmp_enable1), .i_pcmp_mode(i_pcmp_mode1), .i_pcmp_lane_mask(i_pcmp_lane_mask1),
        .i_pcmp_thr_per_lane(i_pcmp_thr_per_lane), .i_pcmp_thr_aggregate(i_pcmp_thr_aggregate),
        .i_pcmp_iter_count(i_pcmp_iter_count1), .i_pcmp_pattern_mode(i_pcmp_pattern_mode1),
        .i_pcmp_clear(i_pcmp_clear1),
        .i_vcmp_enable(i_vcmp_enable1), .i_vcmp_mode(i_vcmp_mode1), .i_vcmp_thr(i_vcmp_thr),
        .i_vcmp_clear(i_vcmp_clear1), .i_clk_detector_en(i_clk_detector_en1), 
        .i_rx_data_deser_en(i_rx_data_deser_en1), .i_rx_valid_deser_en(i_rx_valid_deser_en1),
        // RX in <- die0 TX
    ...
    function automatic bit both_clk_pass();
        return (repairclk_rckp_pass0 && repairclk_rckn_pass0 && repairclk_rtrk_pass0 &&
                repairclk_rckp_pass1 && repairclk_rckn_pass1 && repairclk_rtrk_pass1);
    endfunction
    ...
    function automatic bit both_vcmp_done();return (mb_rx_compare_done0 && mb_rx_compare_done1);      endfunction
    function automatic bit both_pcmp_done();return (mb_rx_compare_done0 && mb_rx_compare_done1);       endfunction
    ...
    task automatic link_reset(input bit embedded_clk);
        ...
        tb_mb_rx_data_en = 0;
        tb_mb_rx_valid_en = 0;
        lp_data0='0; lp_data1='0; lp_irdy=0; lp_valid=0; 
        tb_active = 0;
        tb_mb_tx_pattern_en = 0;
        tb_mb_tx_pattern_setup = 3'b000;
        tb_mb_tx_data_pattern_sel = 2'b00;
        tb_mb_tx_val_pattern_sel = 0;
        tb_mb_rx_compare_en = 0;
        tb_mb_rx_compare_setup = 2'b00;
        tb_clear_error_req = 0;
        tb_mb_lane_reversal_req = tb_reversal_en;
        tb_mb_tx_lfsr_rst = 0;
        tb_mb_rx_lfsr_rst = 0;
        tb_mb_rx_vcomp_mode = 0;
        tb_reg_lane_mask0 = '0;
        tb_reg_lane_mask1 = '0;
        i_clk_embedded_en = embedded_clk;
        
        i_pcmp_thr_per_lane = '0;
        i_pcmp_thr_aggregate = '0;
        i_vcmp_thr = 16'd0;
    ...
    task automatic start_stream(input [1:0] rxmode, input [2:0] lfsr_st);
        ...
        if (lfsr_st == LFSR_DATA) begin
            ...
            tb_mb_tx_lfsr_rst = 0;
            tb_mb_rx_lfsr_rst = 0;
        end else if (lfsr_st == LFSR_PERLANE) begin
            tb_active = 0;
            lp_irdy = 0;
            lp_valid = 0;
            tb_mb_tx_pattern_en = 1;
            tb_mb_tx_pattern_setup = 3'b001; // setup bit 0 set
            tb_mb_tx_data_pattern_sel = 2'b01; // bit 0 = 1 (Per-lane ID)
            tb_mb_tx_lfsr_rst = 0;
            tb_mb_rx_lfsr_rst = 0;
        end else if (lfsr_st == LFSR_PATTERN) begin
            tb_active = 0;
            lp_irdy = 0;
            lp_valid = 0;
            tb_mb_tx_pattern_en = 1;
            tb_mb_tx_pattern_setup = 3'b001; // setup bit 0 set
            tb_mb_tx_data_pattern_sel = 2'b00; // bit 0 = 0 (LFSR/PRBS)
            tb_mb_tx_lfsr_rst = 0;
            tb_mb_rx_lfsr_rst = 0;
        end else if (lfsr_st == LFSR_CLEAR) begin
            tb_active = 0;
            lp_irdy = 0;
            lp_valid = 0;
            tb_mb_tx_pattern_en = 0;
    ...
    task automatic phase_clock_test(input string lbl, input int fault, output bit ok);
        ...
        @(negedge o_pll_clk0);
        tb_mb_tx_pattern_en = 1;
        tb_mb_tx_pattern_setup = 3'b100;
        tb_mb_rx_compare_en = 1;
        tb_mb_rx_compare_setup = 2'b11;
        t = 0;
        while (!(both_clk_pass() && mb_rx_compare_done0 && mb_rx_compare_done1) && t < 4000) begin
            @(posedge o_pll_clk0); t++;
        end
        ok = both_clk_pass() && mb_rx_compare_done0 && mb_rx_compare_done1;
        if (ok) begin
            repeat (20) @(posedge o_pll_clk0);
            if (!(both_clk_pass() && mb_rx_compare_done0 && mb_rx_compare_done1)) begin
                ok = 0;
            end
        end
        $display("  [%s] CLOCK TEST  : d0(p/n/t)=%0b%0b%0b d1(p/n/t)=%0b%0b%0b (%0d)%s",
                 lbl, repairclk_rckp_pass0, repairclk_rckn_pass0, repairclk_rtrk_pass0,
                 repairclk_rckp_pass1, repairclk_rckn_pass1, repairclk_rtrk_pass1, t, ok ? "" : "  <-- FAIL");
    ...
    task automatic phase_valid_test(input string lbl, input bit mode1, input int fault, output bit ok);
        ...
        @(negedge lclk0);
        tb_mb_rx_vcomp_mode = mode1;
        i_vcmp_thr = 16'd0;
        tb_clear_error_req = 1;
        @(negedge lclk0);
        tb_clear_error_req = 0;
        @(negedge lclk0);
        tb_mb_rx_compare_en = 1;
        tb_mb_rx_compare_setup = 2'b10;
        tb_mb_tx_pattern_en = 1;
        tb_mb_tx_pattern_setup = 3'b010;

        t = 0;
        while (!(both_vcmp_done() && mb_tx_pattern_count_done0 && mb_tx_pattern_count_done1) && t < 2000) begin
            @(posedge lclk0); t++;
        end
        ok = (both_vcmp_done() && repairval_RVLD_L_pass0 && repairval_RVLD_L_pass1 && mb_tx_pattern_count_done0 && mb_tx_pattern_count_done1);
        if (ok) begin
            repeat (20) @(posedge lclk0);
            if (!(both_vcmp_done() && repairval_RVLD_L_pass0 && repairval_RVLD_L_pass1 && mb_tx_pattern_count_done0 && mb_tx_pattern_count_done1)) begin
                ok = 0;
            end
        end
        $display("  [%s] VALID  m%0d  : d0(done/pass)=%0b/%0b d1(done/pass)=%0b/%0b (%0d)%s",
                 lbl, mode1, mb_rx_compare_done0, repairval_RVLD_L_pass0,
                 mb_rx_compare_done1, repairval_RVLD_L_pass1, t, ok ? "" : "  <-- FAIL");
    ...
    task automatic phase_data_test(input string lbl, input [1:0] rxmode, input [2:0] lfsr_st,
        ...
        @(negedge lclk0);
        tb_mb_tx_data_pattern_sel = {1'b0, pat_mode};
        ...
        if (both_rx_en()) begin
            ...
            @(negedge lclk0);
            tb_mb_rx_compare_en = 1;
            tb_mb_rx_compare_setup = 2'b01;
            t = 0;
            while (!(both_pcmp_done() && mb_tx_pattern_count_done0 && mb_tx_pattern_count_done1) && t < 800) begin
                @(posedge lclk0); t++;
            end
        end
        ok = (both_pcmp_done() && mb_tx_pattern_count_done0 && mb_tx_pattern_count_done1 &&
              (mb_rx_perlane_pass0 === 16'hFFFF) && (mb_rx_perlane_pass1 === 16'hFFFF));
        if (ok) begin
            repeat (20) @(posedge lclk0);
            if (!(both_pcmp_done() && mb_tx_pattern_count_done0 && mb_tx_pattern_count_done1 &&
                  (mb_rx_perlane_pass0 === 16'hFFFF) && (mb_rx_perlane_pass1 === 16'hFFFF))) begin
                ok = 0;
            end
        end
        $display("  [%s] %-12s: d0 per_lane=0x%04h d1 per_lane=0x%04h%s",
                 lbl, (pat_mode ? "DATA perlane" : "DATA lfsr"),
                 mb_rx_perlane_pass0, mb_rx_perlane_pass1, ok ? "" : "  <-- FAIL");
    ...
    task automatic run_fulltraining_happy_scenario(output bit ok);
        ...
        step_ok = both_clk_pass() && mb_rx_compare_done0 && mb_rx_compare_done1;
        if (!step_ok) begin
            $display("  [FAIL] Clock pattern test failed or timed out. d0(p/n/t)=%0b%0b%0b d1(p/n/t)=%0b%0b%0b, t=%0d",
                     repairclk_rckp_pass0, repairclk_rckn_pass0, repairclk_rtrk_pass0,
                     repairclk_rckp_pass1, repairclk_rckn_pass1, repairclk_rtrk_pass1, t);
            ok = 0; return;
        end
        ...
        step_ok = both_vcmp_done() && repairval_RVLD_L_pass0 && repairval_RVLD_L_pass1 && mb_tx_pattern_count_done0 && mb_tx_pattern_count_done1;
        if (!step_ok) begin
        ...
        repeat (20) @(posedge lclk0);
        if (!(mb_tx_pattern_count_done0 && mb_tx_pattern_count_done1 && repairval_RVLD_L_pass0 && repairval_RVLD_L_pass1 && both_vcmp_done())) begin
        ...
        rx_reset();
        start_stream(RXMODE_DATA, LFSR_CLEAR);
        start_stream(RXMODE_DATA, LFSR_IDLE);
        
        @(negedge lclk0);
        tb_mb_tx_data_pattern_sel = 2'b01; // Per-lane ID selection
        tb_clear_error_req = 1;
        @(negedge lclk0);
        tb_clear_error_req = 0;
        
        start_stream(RXMODE_PERLANE, LFSR_PERLANE);
        tb_mb_rx_compare_en = 1;
        tb_mb_rx_compare_setup = 2'b01; // per-lane comparison
        ...
        if (both_rx_en()) begin
            ...
            t = 0;
            while (!(both_pcmp_done() && mb_tx_pattern_count_done0 && mb_tx_pattern_count_done1) && t < 800) begin @(posedge lclk0); t++; end
        end
        step_ok = both_pcmp_done() && 
                  (mb_rx_perlane_pass0 === 16'hFFFF) && (mb_rx_perlane_pass1 === 16'hFFFF) &&
                  mb_tx_pattern_count_done0 && mb_tx_pattern_count_done1;
        if (!step_ok) begin
            $display("  [FAIL] Data perlane test failed. rx_en=%b, pcmp_done=%b, pass0=%h, pass1=%h",
                     both_rx_en(), both_pcmp_done(), mb_rx_perlane_pass0, mb_rx_perlane_pass1);
            ok = 0; return;
        end
        $display("  [PASS] Data perlane test complete.");

        repeat (20) @(posedge lclk0);
        if (!(mb_tx_pattern_count_done0 && mb_tx_pattern_count_done1 && both_pcmp_done() && mb_rx_perlane_pass0 === 16'hFFFF && mb_rx_perlane_pass1 === 16'hFFFF)) begin
        ...
        @(negedge lclk0);
        tb_mb_tx_data_pattern_sel = 2'b00; // LFSR selection
        i_pcmp_thr_per_lane = 16'd5;
        tb_clear_error_req = 1;
        @(negedge lclk0);
        tb_clear_error_req = 0;
        
        start_stream(RXMODE_PATTERN, LFSR_PATTERN);
        tb_mb_rx_compare_en = 1;
        tb_mb_rx_compare_setup = 2'b01;
        ...
        rx_reset();
        @(negedge lclk0);
        tb_mb_rx_vcomp_mode = 1'b1; // threshold mode
        i_vcmp_thr = 16'd5;
        tb_clear_error_req = 1;
        @(negedge lclk0);
        tb_clear_error_req = 0;
        ...
        @(negedge lclk0);
        tb_mb_tx_data_pattern_sel = 2'b01;
        tb_clear_error_req = 1;
        @(negedge lclk0);
        tb_clear_error_req = 0;
    ...
    initial begin
        ...
        $display("\n============ MB TWO-DIE (die0 <-> die1) LTSM TRAINING SEQUENCE ============");
        ...
        $display("\n=========================================================");
        $display("  unit_mb_die2die_ltsm : %0d scenarios passed, %0d failed", scenarios_pass, scenarios_fail);
        if (scenarios_fail == 0)
